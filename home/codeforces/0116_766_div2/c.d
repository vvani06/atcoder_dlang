...
void problem() {
  ...
  auto solve() {
    ...
    auto subSolve() {
      auto N = scan!int;
      auto E = scan!int(2 * (N - 1)).chunks(2).array;
      ...
      if (g.any!"a.length > 2") return "-1";

      auto ans = new int[](N - 1);
      void dfs(int cur, int pre, int preNum) {
        foreach(e; g[cur]) {
          if (e.to == pre) continue;

          ans[e.id] = preNum == 2 ? 3 : 2;
          dfs(e.to, cur, preNum == 2 ? 3 : 2);
        }
      }

      foreach(e; g[0]) dfs(e.to, -1, 2);
