...
void problem() {
  auto H = scan!int;
  auto W = scan!int;
  auto C = scan!int(H * W).chunks(W).array;

  auto solve() {
    auto ans = new int[][](0, 3);
    auto painted = new int[][](H, W);
    foreach(y; iota(0, H, 1)) foreach(x; iota(0, W, 1)) {
      if (x == W - 1 || y == H - 1) continue;

      const c = C[y][x];
      if (painted[y][x] == c) continue;
    ...
    ans.length.writeln;
    ans.each!(a => a.toAnswerString.writeln);
  }
