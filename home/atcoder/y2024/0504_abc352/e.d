...
void problem() {
  ...
  auto solve() {
    ...
    foreach(_; 0..M) {
      auto K = scan!int;
      auto C = scan!long;
      auto A = scan!int(K).map!"a - 1".array;
      cqs ~= Clique(A, C);
    }

    long ans;
    UnionFind uf = UnionFind(N);
    foreach(cq; cqs.sort!"a.cost < b.cost") {
      auto base = cq.nodes[0];
      foreach(a; cq.nodes[1..$]) {
        if (!uf.same(base, a)) {
          uf.unite(base, a);
          ans += cq.cost;
        }
    ...
    if (N.iota.map!(a => uf.root(a)).array.sort.uniq.walkLength != 1) return -1;
    return ans;
  }
