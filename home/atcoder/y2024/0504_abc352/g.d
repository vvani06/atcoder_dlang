...
void problem() {
  ...
  auto solve() {
    enum long MOD = 2L^^61 - 1;
    alias MInt = ModInt!MOD;
    MInt base = MInt(uniform(2L, MOD - 1));

    MInt[] rh = [MInt(0)];
    foreach(c; "abcabc") {
      rh ~= rh[$ - 1] * base + MInt(c.hashOf(seed));
    }

    rh[3].deb;
    ((rh[3] - rh[0]) / base^^0).deb;    
    ((rh[6] - rh[3]) / base^^3).deb;
