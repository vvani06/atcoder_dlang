...
void problem() {
  auto N = scan!int;
  auto X = scan!int;
  auto P = scan!real(N).map!"a / 100.0";

  auto solve() {
    real[][] memo = new real[][](N + 1, N + 1);
    foreach(ref m; memo) m[] = 0;
    // foreach(i; 0..N + 1) memo[i][0] = P.fold!"a * (1.0L - b)"(1.0L);

    memo[0][0] = 1;
    foreach(to, p; P.enumerate(1)) {
      foreach(from; 0..to) {
        memo[to][from + 0] += memo[to - 1][from] * (1 - p);
        memo[to][from + 1] += memo[to - 1][from] * p;
      }
    }
    
    real evPerOneTry = 0;
    foreach(count, p; memo[$ - 1]) evPerOneTry += count * p;
