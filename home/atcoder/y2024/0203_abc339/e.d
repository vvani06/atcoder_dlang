...
void problem() {
  auto N = scan!int;
  auto D = scan!int;
  auto A = scan!int(N).map!(a => a + D).array;

  auto solve() {
    auto LIM = A.maxElement + D;
    auto maxTree = SegTree!("max(a, b)", int)(0.repeat(LIM).array);
    
    foreach(i, a; A.enumerate(0)) {
      auto maxi = maxTree.sum(a - D, a + D + 1);
      // [i, a, a-D, a+D, maxi].deb;
      maxTree.update(a, maxi + 1);
    }

    return maxTree.sum(0, LIM + 1);
  }
