...
void problem() {
  ...
  auto solve() {
    ...
    void dfs(int cur) {
      ...
      cis[cur] = ci;
      ofs[cur] = cycles[ci].length.to!int;
      cycles[ci] ~= cur;
      dfs(A[cur - 1]);
    }
    ...
    foreach(i; 1..N + 1) {
      auto num = A[i - 1];
      auto cycle = cycles[cis[num]];
      auto offset = ofs[num];
      long cycleSize = cycle.length;

      ulong move = powmod(2UL, K.unsigned, cycleSize.unsigned) - 1UL;
      long newOffset = (offset + move) % cycleSize;
      ans[i - 1] = cycle[newOffset];
    }
