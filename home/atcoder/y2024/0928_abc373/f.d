...
void problem() {
  auto N = scan!int;
  auto X = int.min.to!long ~ scan!long(N) ~ int.max.to!long;
  auto Q = scan!int;
  auto TG = scan!int(2 * Q).chunks(2);

  auto solve() {
    X.deb;
    long[] xspans;
    foreach(i; 0..N + 1) xspans ~= X[i + 1] - X[i];
    auto spans = LazySegtree!("a + b", long)(xspans, 0L, 0L);

    long leftest = int.min;
    foreach(tg; TG[0..1]) {
      auto t = tg[0];
      auto g = tg[1];

      auto cur = spans.sum(0, t);
      cur.deb;
    }
  }
