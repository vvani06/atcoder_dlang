...
void problem() {
  auto H = scan!int;
  auto W = scan!int;
  auto S = scan!string(H).map!array.array;
  enum DIRS = zip([0, -1, 0, 1], [-1, 0, 1, 0]);
  ...
  auto solve() {
    foreach(y; 0..H) foreach(x; 0..W) {
      if (S[y][x] == '#') {
        foreach(dy, dx; DIRS) {
          auto ny = y + dy;
          auto nx = x + dx;
          if (min(ny, nx) < 0 || ny >= H || nx >= W) continue;

          if (S[ny][nx] == '.') S[ny][nx] = 't';
        }
      }
    }
    ...
    struct Coord {
      int x, y;
      int id(int x, int y) { return y * W + x; }
      Coord fromId(int t) { return Coord(t / W, t % W); }
    }

    foreach(sy; 0..H) foreach(sx; 0..W) {
      if (S[sy][sx] != '.') continue;

      foreach(dy, dx; DIRS) {
        auto y = sy + dy;
        auto x = sx + dx;
        if (min(x, y) < 0 || x >= W || y >= H || S[y][x] != '.') continue;

        uf.unite(id(sx, sy), id(x, y));
      }
    }
    ...
    foreach(sy; 0..H) foreach(sx; 0..W) {
      ...
      foreach(dy, dx; DIRS) {
        ...
        if (S[y][x] == 't') {
          blocks[root][id(x, y)] = true;
        }
      }
    }

    // '.'.repeat(1000).to!string.deb;
    // '#'.repeat(1000).to!string.deb;

    int ans = 1;
    foreach(k, v; sizes) {
      if (k in blocks) v += blocks[k].length.to!int;
      ans = max(ans, v);
    }
    return ans;
  }
