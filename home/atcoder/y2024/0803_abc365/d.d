...
void problem() {
  ...
  auto solve() {
    ...
    foreach(c; S) {
      ...
      foreach(from; "RPS") foreach(to; "RPS") {
        if (from == to) continue;
        if (to == 'R' && c == 'P') continue;
        if (to == 'P' && c == 'S') continue;
        if (to == 'S' && c == 'R') continue;

        int add = to == c ? 0 : 1;
        memo[to].chmax(pre[from] + add);
      }
    }

    return memo.values.maxElement;
  }
