...
void problem() {
  ...
  auto DC = M.iota.map!(_ => Move(scan, scan!long)).array;

  auto solve() {
    RedBlackTree!long[long] cPerR, rPerC;
    foreach(xy; XY) {
      auto x = xy[0];
      auto y = xy[1];
      cPerR.require(y, new long[](0).redBlackTree);
      cPerR[y].insert(x);
      rPerC.require(x, new long[](0).redBlackTree);
      rPerC[x].insert(y);
    }
    ...
    foreach(move; DC) {
      ...
      if (move.d == "U" || move.d == "D") {
        ...
        auto moved = X;
        if (move.d == "R") moved += move.c; else moved -= move.c;
