...
void problem() {
  ...
  auto solve() {
    long[ulong][] countsPerHash;
    countsPerHash.length = S.map!"a.length".maxElement + 1;

    foreach(s; S) {
      auto rh = RollingHash(s);
      foreach(i; 1..s.length + 1) {
        countsPerHash[i][rh.get(0, i)]++;
      }
    }


    long ans;
    foreach(long l, c; countsPerHash) {
      auto pairs = c.values.map!"a * (a - 1) / 2".sum;
      ans += pairs;
    }
    return ans;
  }
