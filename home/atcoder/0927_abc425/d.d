...
void problem() {
  auto H = scan!int();
  auto W = scan!int();
  auto G = scan!string(H).map!(s => s.array).array;
  ...
  auto solve() {
    ...
    int countAround(Coord coord) {
      int ret;
      foreach(next; around(coord)) {
        if (G[next.r][next.c] == '#') ret++;
      }
      return ret;
    }

    auto queue = new Coord[](0).redBlackTree;
    auto visited = new bool[][](H, W);
    foreach(r; 0..H) foreach(c; 0..W) {
      if (G[r][c] == '.') continue;

      queue.insert(around(Coord(r, c)));
      visited[r][c] = true;
    }
    ...
    while(!queue.empty) {
      ...
      foreach(cur; queue.array) {
        if (countAround(cur) != 1) {
          continue;
        }
      ...
      foreach(f; fillers.keys) G[f.r][f.c] = '#';

      queue.clear();
      foreach(coord; nexts.keys) {
        queue.insert(coord);
