...
void problem() {
  ...
  auto solve() {
    ...
    foreach(long i, a; A) {
      foreach(k; okA.keys()) {
        const m = (k + a) % 200;
        if (m in okA && !okA[m].empty) {
          // ans
          YESNO[true].writeln;
          
          auto ansArrayA = (okA[m].length - 1) ~ okA[m];
          auto ansArrayB = (okA[k].length) ~ okA[k] ~ [i];
          ansArrayA.map!"(a + 1).to!string".joiner(" ").writeln;
          ansArrayB.map!"(a + 1).to!string".joiner(" ").writeln;
