...
void problem() {
  ...
  Point[] splitRect(Point rect, long seg) {
    Point[] splitted;
    if (rect.x == 0 || rect.y == 0) return splitted;

    if ((seg + rect.x - 1) / rect.x <= rect.y) {
      splitted ~= Point(rect.x, rect.y - (seg + rect.x - 1) / rect.x);
    }

    if ((seg + rect.y - 1) / rect.y <= rect.x) {
      splitted ~= Point(rect.y, rect.x - (seg + rect.y - 1) / rect.y);
    }
  ...
  auto solve() {
    foreach(comb; [A, B, C].permutations) {
      Point[] rects = [Point(X, Y)];
      foreach(seg; comb) {
        Point[] nex;
        foreach(r; rects) nex ~= splitRect(r, seg);

        rects = nex;
      }

      if (!rects.empty) return YESNO[true];
    }
