...
void problem() {
  ...
  auto GX = scan!int - 1;
  auto GY = scan!int - 1;
  auto WALL = scan!string(N).map!(s => s.map!(c => c == '#').array).array;
  enum INF = int.max / 2;

  auto solve() {
    auto visited = new bool[][](N, N);
    auto grid = new int[][](N, N);
    foreach(ref g; grid) g[] = INF;
    
    int step;
    for(auto q = new DList!Point([Point(SY, SX)]); !q.empty;) {
      Point[] next;
      ...
      while(!q.empty) {
        ...
        static foreach(int[] m; [[1, 1], [-1, 1], [1, -1], [-1, -1]]) {{
          auto v = p;
          v.x += m[0];
          v.y += m[1];
          while(min(v.x, v.y) >= 0 && max(v.x, v.y) < N) {
            if (WALL[v.y][v.x]) break;
            if (grid[v.y][v.x] < step + 1) break;
            grid[v.y][v.x] = step + 1;
            next ~= Point(v);
            v.x += m[0];
            v.y += m[1];
