...
void problem() {
  ...
  auto Q = scan!int;
  auto A = scan!int(N);
  auto LRV = scan!int(3 * Q).chunks(3);

  auto solve() {
    alias LST = TypicalLazySegTree!int;
    // auto segtree = LST.MinUpdate(A);
    auto segtree = LST.SumUpdate(LST.withSize(A));
    
    segtree.prod(0, N).deb;
    foreach(lrv; LRV) {
      segtree.apply(lrv[0] - 1, lrv[1], lrv[2]);
      segtree.prod(0, N).deb;
    }
  }
