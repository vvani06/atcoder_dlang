...
void problem() {
  ...
  auto bits = N.iota.map!"2^^a".array;
  ...
  auto solve() {
    ...
    auto dp = new long[][](total, N);
    auto used = new bool[](total);
    foreach(ref d; dp) d[] = uint.max;
    used[0] = true;
    foreach(i, b; bits) {
      dp[b][i] = 1;
      used[b] = true;
    ...
    int rest = total - N - 1;
    bool[int] updated;
    foreach(b; bits) updated[b] = true;
    long cost = 1;
    long ans = N;
    while(rest > 0) {
      cost++;
      bool[int] next;
      foreach(from; updated.keys) foreach(fromNode; 0..N) {
        if (dp[from][fromNode] == uint.max) continue;

        foreach(toNode; g[fromNode]) {
          auto to = from ^ bits[toNode];
          if (dp[to][toNode].chmin(cost)) {
            if (!used[to]) {
              ans += cost;
              used[to] = true;
              rest--;
      ...
      // next.deb;
      updated = next;
      if (updated.empty) throw new Exception("nop!");
    }
