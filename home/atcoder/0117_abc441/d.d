...
void problem() {
  ...
  auto solve() {
    ...
    Cursor[][] graph = new Cursor[][](N, 0);
    foreach(u, v, c; E.asTuples!3) {
      u--;
      v--;
      graph[u] ~= Cursor(v, c);
    }

    auto queue = DList!Cursor([Cursor(0, 0)]);
    bool[Cursor] nexts;
    foreach(step; 0..L) {
      nexts.clear;
      ...
      foreach(cur; queue) {
        foreach(edge; graph[cur.node]) {
          auto cost = cur.cost + edge.cost;
          if (cost > T) continue;

          nexts[Cursor(edge.node, cost)] = true;
        }
      }
    ...
    foreach(k; nexts.keys) {
      if (S <= k.cost && k.cost <= T) candidates[k.node] = true;
    }
    return iota(1, N + 1).filter!(i => candidates[i - 1]);
  }
