...
void problem() {
  ...
  auto solve() {
    ...
    foreach(ref r; rs.values) r.sort;
    foreach(ref r; ls.values) r.sort;

    foreach(i, p; P) {
      if (D[i] == 'R') {
        if (p.y in ls && !ls[p.y].assumeSorted.upperBound(p.x).empty) return YESNO[true];
      } else {
        if (p.y in rs && !rs[p.y].assumeSorted.lowerBound(p.x).empty) return YESNO[true];
      }
    }
