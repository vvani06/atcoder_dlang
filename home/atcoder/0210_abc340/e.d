...
void problem() {
  ...
  auto solve() {
    auto segtree = SegTree!("a + b", long)(new long[](N + 1), 0);

    foreach(b; B) {
      int c = 0;
      auto lsum = segtree.sum(0, b + 1);
      auto balls = A[b] + lsum;
      A[b] = 0;
      if (balls == 0) continue;

      segtree.update(b, segtree.get(b) - lsum);
      segtree.update(b + 1, segtree.get(b + 1) + lsum);

      auto geta = balls / N;
      balls %= N;
      segtree.update(0, segtree.get(0) + geta);
      // [b, c, balls].deb;

      auto start = (b + c + 1) % N;
      auto end = (start + balls.to!int) % N;
      c = (c + balls) % N;
      // [start, end].deb;

      segtree.update(start, segtree.get(start) + 1);
      if (start <= end) {
        segtree.update(end, segtree.get(end) - 1);
      }
      if (start > end) {
        segtree.update(0, segtree.get(0) + 1);
        segtree.update(end, segtree.get(end) - 1);
      }
      // N.iota.map!(i => A[i] + segtree.sum(0, i + 1)).deb;
    }

    return N.iota.map!(i => A[i] + segtree.sum(0, i + 1));
  }
