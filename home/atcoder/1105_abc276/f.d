...
void problem() {
  ...
  struct Item {
    ...
    Item add(Item other) {
      return Item(0, count + other.count, sum + other.sum);
    }

    Item incr() {
      return Item(number, count + 1);
    }
  }

  auto solve() {
    const maxi = A.maxElement + 1;
    auto items = maxi.iota.map!(i => Item(i, 0)).array;
    auto segtree = SegTree!("a.add(b)", Item)(items, Item(0, 0));

    MInt9 ans;
    foreach(i, a; A) {
      ans += MInt9(a);
      ans += MInt9(segtree.sum(0, a).count * 2 * a);
      ans += segtree.sum(a, maxi).sum * MInt9(2);
      writeln(ans / MInt9((i + 1)^^2));

      segtree.update(a, segtree.get(a).incr);
    }
  }
