...
void problem() {
  auto N = scan!int;
  auto A = scan!long(N ^^ 3).chunks(N ^^ 2).map!(c => c.chunks(N).array).array;
  auto QN = scan!int;
  auto Q = scan!int(6 * QN).chunks(6).array;

  auto solve() {
    auto accYZ = N.iota.map!(x => SegmentAcc!long(N, N, A[x])).array;
    ...
    foreach(q; Q) {
      ...
      writeln(iota(lx, rx).map!(x => accYZ[x].sum(ly, lz, ry, rz)).sum);
    }
