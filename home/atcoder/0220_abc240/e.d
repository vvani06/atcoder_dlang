...
void problem() {
  ...
  auto solve() {
    ...
    long[][] ans = new long[][](N, 0);
    long[] tour;
    long[] tourIn = new long[](N);
    long[] tourOut = new long[](N);

    auto visited = new bool[](N);
    long dfs(long cur, long from, long num = 0) {
      if (visited[cur]) return num;
      visited[cur] = true;
      ...
      foreach(next; graph[cur]) {
        ...
        maxi = max(maxi, dfs(next, cur, maxi + i));
        if (i == 0) i++;
      }
    ...
    return tourIn.zip(tourOut).array.map!"[a[0] + 1, a[1] + 1]";
  }
