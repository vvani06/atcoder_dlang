...
void problem() {
  auto N = scan!byte;
  auto N2 = cast(byte)(N * 2);

  auto solve() {
    auto A = new long[][](0, 0);
    foreach_reverse(t; 1..N2) {
      A ~= 0L.repeat(N2 - t).array ~ scan!long(t);
    }
    // A.each!deb;

    long ans;
    bool[byte] avl;
    foreach(i; 0..N2) avl[cast(byte)i] = true;

    void dfs(long score, byte rest) {
      auto arr = avl.keys;
      if (rest == 2) {
        const u = min(arr[0], arr[1]);
        const v = max(arr[0], arr[1]);
        ans = max(ans, score ^ A[u][v]);
        return;
      }
      
      const i = 0;
      foreach(j; i+1..rest) {
        const u = min(arr[i], arr[j]);
        const v = max(arr[i], arr[j]);
        avl.remove(u);
        avl.remove(v);
        dfs(score ^ A[u][v], cast(byte)(rest - 2));
        avl[u] = true;
        avl[v] = true;
      }
    }
