...
void problem() {
  ...
  auto solve() {
    ...
    long calcValue(long from, long to) {
      const ret = H[from] - H[to];
      [from, to, ret].deb;
      return ret < 0 ? ret*2 : ret;
    }

    long ans;
    auto values = new long[](N);
    values[] = long.min / 2;
    alias EV = Tuple!(long, "node", long, "value");
    for(auto q = [EV(0, 0)].heapify!"a.value < b.value"; !q.empty;) {
      auto p = q.front;
      q.removeFront;
      if (p.value <= values[p.node]) continue;
      values[p.node] = p.value;
      ans = ans.max(p.value);

      foreach(nex; g[p.node]) {
        const v = p.value + calcValue(p.node, nex);
        if (v <= values[nex]) continue;
