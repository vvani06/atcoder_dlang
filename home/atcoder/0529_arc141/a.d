...
void problem() {
  ...
  auto solve() {
    foreach(n; N) {
      ulong l = n.to!string.length;
      ulong ans;
      foreach(times; 2..l + 1) {
        ulong repeatNumber(ulong x) {
          ulong keta = 10 ^^ x.to!string.length;
          return reduce!((r, t) => r*keta + x)(0UL, times.iota);
        }
        ulong t = binarySearch(&repeatNumber, (ulong x) => x <= n, 0, 10UL^^(18 / times));
        ans = max(ans, repeatNumber(t));
        // [times, repeatNumber(t)].deb;
      }
