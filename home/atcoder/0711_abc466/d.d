...
void problem() {
  ...
  auto solve() {
    ...
    foreach(r, c; RC.asTuples!2) {
      ans -= perR[r].length;
      ans -= perC[c].length;
      if (c in perR[r]) ans++;

      foreach(cc; perR[r].keys) perC[cc].remove(r);
      foreach(rr; perC[c].keys) perR[rr].remove(c);
      perR[r].clear;
      perC[c].clear;

      perR[r][c] = perC[c][r] = true;
      ans++;

      // [perR[1..$], perC[1..$]].deb;
      // [perR[r].length, perC[c].length].deb;
      // ans.deb;
    }
