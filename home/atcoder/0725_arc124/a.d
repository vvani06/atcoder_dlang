...
void problem() {
  auto N = scan!long;
  auto K = scan!long;
  auto CK = K.iota.map!(k => tuple(scan!char, scan!long)).array;

  auto solve() {
    auto fixed = new bool[](N);
    auto cards = new long[](N);
    ...
    foreach(ck; CK) {
      ...
      if (c == 'L') {
        foreach(i; k + 1..N) if (!fixed[i]) cards[i]++;
      } else {
        foreach(i; 0..k) if (!fixed[i]) cards[i]++;
      }
    }

    enum long MOD = 998244353;
    long ans = 1;
    foreach(c; cards) ans = (ans * c) % MOD; 
