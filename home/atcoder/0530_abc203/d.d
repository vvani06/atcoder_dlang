...
void problem() {
  auto N = scan!long;
  auto K = scan!long;
  auto A = scan!long(N * N);

  auto solve() {
    const mediumIndex = K^^2/2 + 1;

    bool subSolve(long median) {
      auto acc = new long[][](N + 1, N + 1);
      long i = 1;
      foreach(line; A.map!(a => a >= median ? 1 : 0).chunks(N)) {
        long j = 1;
        foreach(a; line) {
          acc[i][j] = acc[i][j - 1] + acc[i - 1][j] + a;
          j++;
        }
        i++;
      }
      ...
      const s = N - K + 1;
      foreach(y; 0..s) {
        if (acc[y + K][K] - acc[y][0] < mediumIndex) return false;
        foreach(x; 0..s) {
          if (acc[y + K][x + K] - acc[y][x] < mediumIndex) return false;
        }
      }
    ...
    return binarySearch(&subSolve, (bool t) => t, 0L, 10L^^9 + 1);
  }
