...
void problem() {
  auto A = abs(scan!long);
  auto B = abs(scan!long);
  auto X = abs(scan!long);
  auto Y = abs(scan!long);
  ...
  auto solve() {
    ...
    long minXY = min(X, Y);
    long ans = A * 2 * minXY;
    long rest = max(X, Y) - minXY;

    long naiveCost = (rest / 2) * (B + A);
    if (X > Y) {
      naiveCost += (rest % 2) * A;
    } else {
      naiveCost += (rest % 2) * B;
    ...
    long restOne = rest % 2;
    long anotherCost = (rest - restOne) * 2 * A;
    if (X > Y) {
      anotherCost += restOne * A;
    } else {
      anotherCost += restOne * A * 3;
    }

    // [[A, B], [X, Y, rest], [ans, naiveCost, anotherCost]].deb;

    return ans + min(naiveCost, anotherCost);
  }
