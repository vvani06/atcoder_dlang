...
void problem() {
  ...
  auto solve() {
    auto odds = A.filter!"a % 2 == 1".array.sort;
    auto evens = A.filter!"a % 2 == 0".array.sort;

    int ans = -1;
    if (odds.length > 1) ans = max(ans, odds[$ - 1] + odds[$ - 2]);
    if (evens.length > 1) ans = max(ans, evens[$ - 1] + evens[$ - 2]);
