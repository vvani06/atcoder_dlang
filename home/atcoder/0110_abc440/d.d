...
void problem() {
  ...
  auto solve() {
    ...
    foreach(x, y; XY.asTuples!2) {
      auto lowerCount = sorted.lowerBound(x).length.to!long;
      auto uppers = sorted.upperBound(x - 1);
      
      long calcRank(long n) {
        long rank = n - x + 1;
        rank -= uppers.lowerBound(n).length.to!long;
        return rank;
      }
      ...
      auto ans = binarySearch(&isOk, x, 10L^^9 * 3);
      writeln(ans);
    }
  }
