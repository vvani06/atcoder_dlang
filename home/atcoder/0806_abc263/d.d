...
void problem() {
  ...
  auto solve() {
    auto acc = 0 ~ A.cumulativeFold!"a + b".array;

    auto xs = new long[](N + 1);
    foreach(i; 0..N) {
      xs[i + 1] = L * (i + 1) - acc[i + 1];
    }
    xs.deb;

    auto ys = new long[](N + 1);
    foreach(i; 0..N) {
      ys[i + 1] = R * (i + 1) - (acc[N] - acc[N - 1 - i]);
    }
    ys.deb;

    const initialSum = A.sum;
    long ans = long.max / 3;
    long xm;
    auto yt = SegTree!("min(a, b)", long)(ys, 0);
    foreach(x; 0..N + 1) {
      xm = min(xm, xs[x]);
      auto t =  yt.sum(0, N - x + 1);
      ans = min(ans, initialSum + xm + t);
      [x, xm, t, ans].deb;
    }
