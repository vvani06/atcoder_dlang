...
void problem() {
  ...
  auto subSolve(int N, int K, long[] A, long[] B) {
    long[][long] nums;
    foreach(a, b; zip(A, B)) nums[a] ~= b;

    long bsum;
    long fixed;
    auto bs = new long[](0).heapify;
    int k = 1;
    long ans = long.max;
    ...
    foreach(a; nums.keys.sort) {
      nums[a].sort;

      if (fixed != 0) add(fixed);
      fixed = nums[a][0];
      // [[a], [fixed, bsum]].deb;
      if (k == K) ans = min(ans, (fixed + bsum) * a);
  ...
  auto solve() {
    foreach(_; 0..T) {
      auto n = scan!int;
      auto ans = subSolve(n, scan!int, scan!long(n), scan!long(n));
      writeln(ans);
    }
  }
