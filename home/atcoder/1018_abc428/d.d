...
void problem() {
  ...
  auto subSolve() {
    ...
    long ans;
    foreach(digit; 1L..11L) {
      long xMin = max(1L, 10L^^(digit - 1) - C);
      long xMax = min(D, 10L^^(digit) - 1 - C);
      if (xMin > xMax) continue;

      long fMin = (10L^^digit * C) + C + xMin - 1;
      long fMax = (10L^^digit * C) + C + xMax;

      // [fMin, fMax].deb;
      long rootMin = lroot(fMin);
      long rootMax = lroot(fMax);
      // [rootMin, rootMax].deb;
