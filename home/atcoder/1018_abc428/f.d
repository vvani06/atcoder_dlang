...
void problem() {
  auto N = scan!int;
  auto A = scan!long(N);
  auto Q = scan!int.iota.map!(_ => tuple(scan!int, scan!int, scan!long)).array;

  auto solve() {
    auto segtree = TypicalLazySegTree!long.SumAdd(N);
    foreach(i, a; A.enumerate(0)) {
      segtree.apply(i, a);
    }

    foreach(l, r, k; Q.asTuples!3) {
      auto pre = segtree.prod(l - 1, r);
      segtree.apply(l - 1, r, -k);
      auto aft = segtree.prod(l - 1, r);
      writeln(pre);
    }
  }
