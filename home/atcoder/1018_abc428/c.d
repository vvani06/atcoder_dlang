...
void problem() {
  ...
  auto solve() {
    ...
    auto depthes = [0].redBlackTree!true;
    ...
    foreach(_; 0..Q) {
      ...
      if (T == 1) {
        auto C = scan!char;
        queue.insertBack(queue.back + (C == '(' ? 1 : -1));
        depthes.insert(queue.back);
      } else {
        depthes.removeKey(queue.back);
        queue.removeBack();
      }

      // queue.array.deb;
      writeln(depthes.front == 0 && queue.back == 0 ? "Yes" : "No");
    }
  }
