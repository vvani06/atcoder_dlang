...
void problem() {
  ...
  long spread(long len) {
    if (len >= 1000) return 5 - len;
    if (len >= 100) return 4 - len;
    if (len >= 10) return 3 - len;
    return 2 - len;
  }

  auto solve() {
    enum OFS = 3000;
    auto dp = new long[](OFS * 2 + 1);
    auto pre = new long[](OFS * 2 + 1);
    dp[OFS + 1] = 26;
    foreach(i; 1..N) {
      swap(dp, pre);
      dp[] = 0;
      foreach(f; OFS-i..OFS+i+1) {
        dp[f - 1] += pre[f];
        dp[f - 1] %= MOD;
        dp[f + 1] += pre[f] * 25;
        dp[f + 1] %= MOD;
