...
void problem() {
  ...
  auto solve() {
    auto compressed = A.joiner.array.sort.uniq.array;
    int[long] ci;
    foreach(i, c; compressed) ci[c] = i.to!int;
    
    auto values = new int[](N * M);
    foreach(a; A.joiner) values[ci[a]]++;

    auto segtree = SegTree!("a + b", int)(values);

    long ans;
    foreach(i; 0..N - 1) {
      foreach(a; A[i]) {
        segtree.update(ci[a], segtree.get(ci[a]) - 1);
      }

      ans += (N - 1 - i) * M * (M + 1) / 2;
      foreach(a; A[i]) {
        ans += segtree.sum(0, ci[a]);
        // [a, segtree.sum(0, ci[a])].deb;
      }
    }
