...
void problem() {
  ...
  auto subSolve(int N, int R, int C) {
    if (N % 2 == 1 || (R + C) % 2 == 0) {
      writeln(YESNO[false]);
      return;
    }
    ...
    foreach(rh; 0..N / 2) {
      ...
      if (rh < halfR) {
        ans ~= 'R'.repeat(N - 1).to!string ~ 'D' ~ 'L'.repeat(N - 1).to!string;
      } else if (rh == halfR) {
        bool down;
        foreach(c; 0..N - 1) {
          if (c == C) {
        ...
        ans ~= 'L'.repeat(N - 1).to!string ~ 'D' ~ 'R'.repeat(N - 1).to!string;
      }
    }
  ...
  auto solve() {
    foreach(_; 0..T) {
      subSolve(scan!int, scan!int - 1, scan!int - 1);
    }
  }
