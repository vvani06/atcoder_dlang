...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto A = scan!int(N).map!"a - 1".array;
  auto TB = scan!int(2 * Q).chunks(2).map!(tb => [tb[0], tb[1] - 1]).array;

  auto solve() {
    auto p = new int[][](60, N);
    auto s = new long[][](60, N);
    p[0] = N.iota.array;
    p[1] = A.dup;
    s[1] = iota(1L, N.to!long + 1).array;
    foreach(lv; 2..60) foreach(i; 0..N) {
      auto preNode = p[lv - 1][i];
      p[lv][i] = p[lv - 1][preNode];
      s[lv][i] = s[lv - 1][i] + s[lv - 1][preNode];
    }

    long calc(int t, int b) {
      // [t, b].deb;
      long ret;
      foreach(lv; 1..60) {
        if (t % 2) {
