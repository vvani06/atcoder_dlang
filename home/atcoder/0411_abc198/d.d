...
void problem() {
  enum UNSOLVABLE = "UNSOLVABLE";
  auto S = scan!string(3);

  auto solve() {
    bool[char] charMap;
    foreach(c; S[0]~S[1]~S[2]) charMap[c] = true;
    if (charMap.length > 10) {
      UNSOLVABLE.writeln;
      return;
    }

    auto chars = charMap.keys;
    auto l = chars.length.to!int;
    auto NUMS = 10.iota.array;
    long[char] charTable;
    foreach(i, c; chars) charTable[c] = i;
    
    foreach(combBase; NUMS.combinations(l)) {
      foreach(comb; combBase.permutations) {
        if (S.any!(s => comb[charTable[s[0]]] == 0)) continue;

        long numMap(string s) {
          long ret;
          foreach(c; s) ret = ret*10 + comb[charTable[c]];
          return ret;
        }

        auto n = S.map!(numMap);
        if (n[0] + n[1] == n[2]) {
          "%s %s %s".writefln(n[0], n[1], n[2]);
          return;
        }
      }
    }
