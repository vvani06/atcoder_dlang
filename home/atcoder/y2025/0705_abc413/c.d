...
void problem() {
  ...
  auto solve() {
    alias Item = Tuple!(long, "value", long, "count");
    DList!Item queue;
    ...
    foreach(_; 0..Q) {
      ...
      if (t == 1) {
        ...
        while(k > 0) {
          ...
          queue.removeFront();
          auto removed = min(head.count, k);
          k -= removed;
          head.count -= removed;
          ans += head.value * removed;
          if (head.count > 0) queue.insertFront(head);
        }
        writeln(ans);
      }
    }
