...
void problem() {
  ...
  auto solve() {
    ...
    auto recons = DList!(int[])();
    foreach(i, e; E.enumerate(1)) {
      if (uf.same(e[0], e[1])) {
        recons.insertBack([i, e[0], e[1]]);
        continue;
      }
      uf.unite(e[0], e[1]);
    }

    auto roots = iota(1, N + 1).map!(i => uf.root(i)).redBlackTree;
    
    string[] ans;
    foreach(edge; recons) {
      roots.removeKey(uf.root(edge[1]));
      if (roots.empty) break;

      auto other = roots.front;
      roots.removeFront;
      uf.unite(edge[1], other);
      ans ~= "%s %s %s".format(edge[0], edge[1], other);
      roots.insert(uf.root(edge[1]));
    }

    return tuple(ans.length, ans);
  }
