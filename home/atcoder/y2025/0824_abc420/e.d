...
struct BWSet {
  RedBlackTree!int black;
  RedBlackTree!int white;

  BWSet merge(BWSet other) {
    black.insert(other.black[]);
    white.insert(other.white[]);
    return BWSet(black, white);
  }
}
...
void problem() {
  ...
  auto solve() {
    bool[] blacks = new bool[](N);
    auto uf = UnionFindWith!BWSet(N);
    foreach(i; 0..N) {
      uf.setExtra(i, BWSet(new int[](0).redBlackTree, [i].redBlackTree));
    }
    ...
    foreach(_; 0..Q) {
      ...
      if (T == 1) {
        ...
        auto ex = uf.extra(V);
        if (blacks[V]) {
          ex.black.removeKey(V);
          ex.white.insert(V);
        } else {
          ex.white.removeKey(V);
          ex.black.insert(V);
        }
        uf.setExtra(V, ex);
        blacks[V] ^= true;
        ...
        auto V = scan!int - 1;
        writeln(uf.extra(V).black.empty ? "No" : "Yes");
      }
    }
  }
