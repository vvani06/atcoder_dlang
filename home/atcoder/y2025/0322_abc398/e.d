...
void problem() {
  auto N = scan!int;
  auto UV = scan!int(2 * N - 2).chunks(2).array;
  
  auto solve() {
    alias Pair = Tuple!(int, "i", int, "j");
    auto stock = new Pair[](0).redBlackTree;
    ...
    foreach(start; 0..N) {
      void dfs(int cur, int pre, int depth) {
        if (depth > 1 && (depth % 2 == 1)) {
          auto l = min(start, cur);
          auto r = max(start, cur);
          stock.insert(Pair(l + 1, r + 1));
        }
    ...
    bool first = stock.array.length % 2 == 1;
    ...
    while(true) {
      auto removed = Pair(scan!int, scan!int);
      if (removed.i == -1) {
        break;
      }

      stock.removeKey(removed);
