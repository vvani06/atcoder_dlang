...
void problem() {
  ...
  struct Combo {
    ...
    Combo merge(Combo other) {
      int ls = mono() && left.c == other.left.c ? size + other.left.size : left.size;
      int rs = other.mono() && right.c == other.left.c ? other.size + right.size : other.right.size;
      int bs = max(best.size, other.best.size, ls, rs, right.c == other.left.c ? right.size + other.left.size: 0);

      return Combo(
        Element(left.c, ls),
        Element('_', bs),
        Element(other.right.c, rs),
        size + other.size,
      );
    }
  }
  ...
  auto solve() {
    ...
    // foreach(_; 0..10) {
    //   auto s = "sdglkhasldkguhasldkugdsjkgfjhfgaksdjhfgaksjdfghktaghwdkfjhagdsfhjgsfhjagsdjhfgawd".repeat(1000).joiner.array.randomShuffle;
    //   auto naive = s.group.map!"a[1]".maxElement;
    //   auto st = SegTree!("a.merge(b)", Combo)(s.map!(c => fromChar(c)).array, noCombo);
    //   deb([naive, st.sum(0, s.length.to!int).best.size]);
    // }

    auto segtree = SegTree!("a.merge(b)", Combo)(S.map!(c => fromChar(c)).array, noCombo);
    ...
    foreach(_; 0..Q) {
      ...
      if (t == 1) {
        auto i = scan!int;
        auto x = scan!dchar;
        segtree.update(i - 1, fromChar(x));
      } else {
        auto l = scan!int;
        auto r = scan!int;
        segtree.sum(l - 1, r).best.size.writeln;
      }
    }
  }
