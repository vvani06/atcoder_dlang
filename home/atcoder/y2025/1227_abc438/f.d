...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto XY = scan!long(N * 2).chunks(2).array;

  auto solve() {
    auto uArr = XY.map!(xy => xy[0] - xy[1]).map!"[a, a]".array;
    auto vArr = XY.map!(xy => xy[0] + xy[1]).map!"[a, a]".array;
    auto utree = SegTree!("[min(a[0], b[0]), max(a[1], b[1])]", long[])(uArr);
    auto vtree = SegTree!("[min(a[0], b[0]), max(a[1], b[1])]", long[])(vArr);
    ...
    foreach(_; 0..Q) {
      ...
      if (t == 1) {
        ...
        auto x = scan!long;
        auto y = scan!long;
        utree.update(i, [x - y, x - y]);
        vtree.update(i, [x + y, x + y]);
      } else {
        auto l = scan!int - 1;
        auto r = scan!int;
        auto x = scan!long;
        ...
        auto um = utree.sum(l, r);
        auto vm = vtree.sum(l, r);
        auto u = x - y;
        auto v = x + y;
        writeln(max(
          abs(u - um[0]),
