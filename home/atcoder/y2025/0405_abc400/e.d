...
void problem() {
  ...
  auto solve() {
    auto erat = Eratosthenes(10 ^^ 6);
    auto primes = erat.primes();

    auto n400 = new long[](0).redBlackTree;
    auto ql = new long[](0).redBlackTree;
    foreach(p; primes) {
      long[] qlq;
      for(long k = 2; p^^k <= LIM; k += 2) {
        auto pk = p^^k;
        n400.insert(ql.lowerBound(LIM / pk + 1).map!(a => pk * a));
    ...
    foreach(a; A) {
      writeln(n400.lowerBound(a + 1).back);
    }
  }
