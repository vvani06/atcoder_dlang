...
void problem() {
  auto N = scan!int;
  auto X = scan!int;
  auto VAC = scan!long(3 * N).chunks(3);

  auto solve() {
    auto memo = new long[][](3, X + 1);

    foreach(v; 0..3) {
      foreach(ac; VAC.filter!(a => a[0] == v + 1)) {
        foreach(from; X.iota.retro) {
          auto to = from + ac[2];
          if (to > X) continue;

          memo[v][to].chmax(memo[v][from] + ac[1]);
        }
      }
    }
    ...
    foreach(v; 1..3) {
      auto pre = ans.dup;
      ans[] = 0;
      foreach(fc; 0..X + 1) foreach(tc; 0..X + 1) {
        auto sc = fc + tc;
        if (sc > X) break;

        ans[sc].chmax(min(pre[fc], memo[v][tc]));
      }
    }
