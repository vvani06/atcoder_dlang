...
void problem() {
  ...
  auto solve() {
    int[int][] edges;
    edges.length = N + 1;
    int[][] nodes = new int[][](N + 1, 0);
    int[] rev = new int[](N + 1);

    auto xyi = zip(XY.map!"a[0]", XY.map!"a[1]", iota(1, N + 1)).array;
    int nodeCursor;
    foreach(from, cost, xyCursor; xyi.asTuples!3) {
      auto realFrom = rev[from];
      if (!(cost in edges[realFrom])) {
        nodeCursor++;
        edges[realFrom][cost] = nodeCursor;
        nodes[nodeCursor] ~= xyCursor;
        rev[xyCursor] = nodeCursor;
      } else {
        auto preNode = edges[realFrom][cost];
        nodes[preNode] ~= xyCursor;
        rev[xyCursor] = preNode;
      }
    }
    ...
    void dfs(int cur) {
      ans ~= nodes[cur];
      foreach(cost; edges[cur].keys.dup.sort) {
        dfs(edges[cur][cost]);
      }
    }
    dfs(0);
