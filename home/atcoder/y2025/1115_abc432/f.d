...
void problem() {
  auto N = scan!int;
  auto G = scan!string(3).map!(s => s.map!(c => c == '.').array).array;
  auto Q = scan!int.iota.map!(_ => tuple(scan!int - 1, scan!int - 1)).array;

  auto solve() {
    bool[] state = G.joiner.array;
    foreach(r, c; Q.asTuples!2) state[r * N + c] = false;

    foreach(r, c; Q.asTuples!2) {
      auto ba = BitArray(state);
