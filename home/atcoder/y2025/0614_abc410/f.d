...
void problem() {
  ...
  auto solve() {
    alias Edge = Tuple!(int, "id", int, "to");
    Edge[][] graph = new Edge[][](N, 0);
    foreach(id, e; E) {
      graph[e[0]] ~= Edge(id.to!int, e[1]);
      graph[e[1]] ~= Edge(id.to!int, e[0]);
    }

    int[] nodeIdonRoute = new int[](N);
    int[] edgeIdonRoute = new int[](N - 1);
    int[] edgeIdoffRoute = new int[](N - 1);
    long[] tours = [1L];

    void dfs(int cur, int from) {
      nodeIdonRoute[cur] = tours.length.to!int - 1;
      foreach(e; graph[cur]) {
        if (e.to == from) continue;
        
        edgeIdonRoute[e.id] = tours.length.to!int;
        tours ~= 1L;
        dfs(e.to, cur);
        edgeIdoffRoute[e.id] = tours.length.to!int;
      }
    }
    dfs(0, 0);

    auto segtree = SegTree!("a + b", long)(tours);
    long total = N;
    foreach(_; 0..Q) {
      auto t = scan!int;
      ...
      if (t == 1) {
        ...
        segtree.add(nodeIdonRoute[x], w);
        total += w;
      } else {
        auto y = scan!int - 1;
        auto partial = segtree.sum(edgeIdonRoute[y], edgeIdoffRoute[y]);
        writeln(abs(total - 2*partial));
      }
    }
  }
