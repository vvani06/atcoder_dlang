...
void problem() {
  ...
  auto solve() {
    long ans;
    long times;
    for(auto q = DList!(long[])([A]); !q.empty;) {
      auto arr = q.front; q.removeFront;

      const mini = arr.minElement;
      ans += mini;
      if (arr.maxElement == mini) continue;
      ...
      long[] indicies;
      foreach(i, a; arr) if (a == mini) indicies ~= i;

      if (indicies.filter!(a => a != 0 && a != arr.length - 1).empty) {
        long[] t;
        foreach(a; arr) if (a > mini) t ~= a - mini;
        q.insertBack(t);
      } else {
