...
void problem() {
  ...
  auto solve() {
    ...
    foreach(k; 0..K) {
      ...
      ans += A[total];
      added ~= A[total];
      if (dest[total] != -1) break;

      auto nex = (total + A[total]) % N;
      dest[total] = nex;
      order[total] = k;
      preTotal = total;
      total = nex;
    ...
    dest.deb;
    order.deb;
    auto cycle = order[preTotal] - order[total] + 1;
    cycle.deb;
    added.deb;
    auto cycleAdds = added[$ - cycle..$];
    cycleAdds.deb;
    
    const times = K / cycle;
    K -= times * cycle;
    ans += cycleAdds.sum * times;
    ans += cycleAdds.array[0..K].sum;
