...
void problem() {
  ...
  auto solve() {
    auto available = new bool[][][](11, 10, 256);
    foreach(s; S) foreach(i, c; s.enumerate(0)) {
      available[s.length][i][c] = true;
    }
    ...
    foreach(s; S) {
      ...
      bool ans = true;
      foreach(r, x; s.enumerate(0)) {
        auto l = AB[r][0];
        auto c = AB[r][1];
        if (!available[l][c - 1][x]) ans = false;
      }
      writeln(YESNO[ans]);
    }
  }
