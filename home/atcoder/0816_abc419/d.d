...
void problem() {
  ...
  auto S = scan!string();
  auto T = scan!string();
  auto LR = scan!int(M * 2).chunks(2).array;

  auto solve() {
    int[int] delta;
    delta[N] = 0;
    foreach(l, r; asTuples!2(LR)) {
      delta[l - 1]++;
      delta[r]++;
    }
    ...
    foreach(i; delta.keys.sort) {
      swapped[pre..i] = cur % 2 == 1;
      cur += delta[i];
      pre = i;
    }

    return iota(N).map!(i => swapped[i] ? T[i] : S[i]).to!string;
  }
