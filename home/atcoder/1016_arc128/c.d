...
void problem() {
  ...
  auto solve() {
    auto acc = new real[](N + 1);
    acc[0] = 0;
    foreach(i; 0..N) acc[i + 1] = acc[i] + A[i];

    long r = N;
    real rest = S;
    real ans = 0;
    while (r > 0 && rest > 0.00001) {
      long maxi = -1;
      real maxv = 0;
      real maxx = 0;
      foreach_reverse(i; 1..r) {
        real x = min(M, rest/(r - i));
        real exp = (acc[r] - acc[i]) * x;
        real exo = acc[i] * (rest - x*(r - i))/i;
        deb([r, i], [exp, exo, x]);
        if (exp > exo && maxv.chmax(exp)) {
          maxi = i;
          maxx = x;
          break;
        }
