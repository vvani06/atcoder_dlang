...
void problem() {
  ...
  auto solve() {
    ...
    for(auto queue = DList!Coord(Coord(0, 0)); !queue.empty;) {
      ...
      queued[cur.y][cur.x] = visited[cur.y][cur.x] = true;

      auto from = S[cur.y][cur.x];
      foreach(dx, dy; zip([-1, 0, 1, 0], [0, -1, 0, 1])) {
        auto x = cur.x + dx;
        auto y = cur.y + dy;
        if (min(x, y) < 0 || x >= W || y >= H) continue;
        if (visited[y][x] || queued[y][x]) continue;
