...
void problem() {
  ...
  auto solve() {
    ...
    foreach(y; 0..H) foreach(x; 0..W) {
      foreach(dy, dx; zip([0, 1, 0, -1], [1, 0, -1, 0])) {
        auto ay = y + dy;
        auto ax = x + dx;
        if (min(ay, ax) < 0 || ay >= H || ax >= W) continue;

        if (S[y][x] == '#' && S[y][x] == S[ay][ax]) {
          uf.unite(id(y, x), id(ay, ax));
        }
      }
    }
    ...
    MInt9 rootsCount; {
      ...
      foreach(y; 0..H) foreach(x; 0..W) {
        if (S[y][x] == '#') {
          roots[uf.root(id(y, x))] = true;
        }
      }
      rootsCount = MInt9(roots.length);
    }

    MInt9 totalDiv = MInt9(1) / MInt9(S.map!(s => s.count('.')).sum);
    MInt9 ans;
    ...
    foreach(y; 0..H) foreach(x; 0..W) {
      ...
      foreach(dy, dx; zip([0, 1, 0, -1], [1, 0, -1, 0])) {
        ...
        aroundRoots[uf.root(id(ay, ax))] = true;
      }
      list ~= (rootsCount - (MInt9(aroundRoots.length) - MInt9(1)));
      ans += rootsCount - (MInt9(aroundRoots.length) - MInt9(1));
    }
