...
void problem() {
  auto N = scan!int;
  auto H = scan!long;
  auto M = scan!long(2 * N).chunks(2).array;

  struct Magic {
    long t, d;

    long performance(long turn) {
      return d * min(turn, t);
    }
  }

  auto solve() {
    auto magics = M.map!(a => Magic(a[0], a[1])).array;
    ...
    bool isOk(long tt) {
      ...
      long h;
      while(t > 0) {
        foreach(m; magics.sort!((a, b) => a.performance(t) > b.performance(t))) {
          auto cycle = 1;
          h += m.d * min(t, m.t) * cycle;
          t -= cycle * m.t;
          m.deb;
          break;
        }
