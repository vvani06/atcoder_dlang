...
void problem() {
  ...
  auto solve() {
    ...
    long t = larger - min(A, B);
    long s, ans;

    if (t == 0) return 1;

    while(t < larger) {
      auto sd = s.divisors.assumeSorted;
      foreach_reverse(add; t.divisors) {
        if (add + t > larger || (!sd.empty && !sd.canFind(add))) continue;

        const spread = larger - t;
        const times = 1;
        ans += times;
        t += add * times;
        s += add * times;
        if (add > 1) [add, times, t, s].deb;
        break;
      }
    }
