...
void problem() {
  ...
  auto solve() {
    auto et = new long[](M + 1);
    foreach(ad; A) foreach(a; ad.divisors) {
      if (a == 1 || et[a] > 0) continue;

      for(long x = a; x <= M; x += a) et[x]++;
    }

    et[1..$].count!"a == 0".writeln;
    foreach(i; 1..M + 1) if (et[i] == 0) i.writeln;
  }
