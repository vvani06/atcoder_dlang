...
void problem() {
  auto N = scan!long;
  auto M = scan!long;
  auto AB = scan!long(2 * M).map!"a - 1".chunks(2).array;
  auto CD = scan!long(2 * M).map!"a - 1".chunks(2).array;

  auto solve() {
    auto ag = new bool[][](N, N);
    foreach(ab; AB) {
      ag[ab[0]][ab[1]] = true;
      ag[ab[1]][ab[0]] = true;
    }
    ...
    bool[bool[]] ap;
    foreach(perm; N.iota.permutations) {
      bool[] pat;
      foreach(i; 0..N - 1) foreach(j; i + 1..N) pat ~= ag[perm[i]][perm[j]];
      ap[pat.idup] = true;
    }

    auto cg = new bool[][](N, N);
    foreach(cd; CD) {
      cg[cd[0]][cd[1]] = true;
      cg[cd[1]][cd[0]] = true;
    }
    ...
    foreach(perm; N.iota.permutations) {
      bool[] pat;
      foreach(i; 0..N - 1) foreach(j; i + 1..N) pat ~= cg[perm[i]][perm[j]];
      if (pat in ap) return YESNO[true];
    }
