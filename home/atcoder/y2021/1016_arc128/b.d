...
void problem() {
  auto T = scan!int;
  auto CASE = scan!long(T * 3).chunks(3);

  auto subSolve(long[] arr) {
    long ans = long.max;

    foreach(comb; arr.combinations(2)) {
      const spread = comb[1] - comb[0];
      if (spread % 3 > 0) continue;
    
      ans.chmin(spread/3 + (spread/3)*2 + comb[0]);
    }
