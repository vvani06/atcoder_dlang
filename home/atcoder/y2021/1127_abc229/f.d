...
void problem() {
  ...
  auto solve() {
    ...
    long pre(long p) { return (p - 1 + N) % N; }
    long suf(long p) { return (p + 1) % N; }

    long ans;
    gz[0] = true;
    foreach(n; 1..N) {
      auto costA = A[n] + (gz[pre(n)] ? B[pre(n)] : 0) + (gz[suf(n)] ? B[n] : 0);
      auto costB = (gz[pre(n)] ? 0 : B[pre(n)]) + (gz[suf(n)] ? 0 : B[n]);
