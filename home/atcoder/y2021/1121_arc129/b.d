...
void problem() {
  auto N = scan!long;
  auto Terms = N.iota.map!(_ => Term(scan!long, scan!long)).array;

  long ans;
  auto solve() {
    auto lt = redBlackTree(new long[](0));
    auto rt = redBlackTree(new long[](0));

    foreach(term; Terms) {
      auto lowers = rt.lowerBound(term.l);
      if (!lowers.empty) {
        ans = max(ans, (term.l - lowers.front + 1) / 2);
      }
      auto uppers = lt.upperBound(term.r);
      if (!uppers.empty) {
        ans = max(ans, (uppers.back - term.r + 1) / 2);
      }
