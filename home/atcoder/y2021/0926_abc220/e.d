...
void problem() {
  ...
  auto solve() {
    ...
    auto nodes = new long[](N);
    nodes[0] = 1;
    foreach(i; 1..N) nodes[i] = (nodes[i-1] * 2) % MOD;

    N--;
    if (D > 2*N) return 0;

    long pat = D == 1 ? 0 : (nodes[D / 2 - 1] * nodes[(D + 1) / 2 - 1] * 2) % MOD;
    ...
    foreach(d; 0..N) {
      ...
      const rest = N - d;
      const t = min(D, rest + 1) - max(1, D - rest);
      add += max(0, t) * pat;
      add %= MOD;

      // foreach(i; max(1, D - rest)..min(D, rest + 1)) {
      //   // [d, i, D-i].deb;
      //   add += ((nodes[i - 1]) * (nodes[D - i - 1]) * 2) % MOD;
      //   add %= MOD;
      // }
