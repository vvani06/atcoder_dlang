...
void problem() {
  auto N = scan!long;
  auto M = scan!long;
  auto A = scan!string(N * 2);

  auto solve() {
    alias Player = Tuple!(long, "id", long, "wins");
    Player[] players;
    foreach(i; 0..2*N) players ~= Player(i, 0);

    foreach(m; 0..M) {
      foreach(i; 0..N) {
        const x = i * 2;
        const pa = players[x];
        const pb = players[x + 1];
        const a = A[pa.id][m];
        const b = A[pb.id][m];

        if (a == 'G' && b == 'C') players[x].wins++;
        if (a == 'C' && b == 'G') players[x + 1].wins++;
        if (a == 'C' && b == 'P') players[x].wins++;
        if (a == 'P' && b == 'C') players[x + 1].wins++;
        if (a == 'P' && b == 'G') players[x].wins++;
        if (a == 'G' && b == 'P') players[x + 1].wins++;
      }
      players.multiSort!("a.wins > b.wins", "a.id < b.id");
    }
