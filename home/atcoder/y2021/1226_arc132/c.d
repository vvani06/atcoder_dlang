  ...
  auto solve() {
    ...
    foreach(long i, a; A) {
      ...
      if (a != -1) {
        ...
        foreach(t; max(1, ii - D)..min(N, ii + D) + 1) {
          if (!(t in memberA)) g[i] ~= t;
        }
      }
    }
    // g.deb;
    ...
    foreach(long i, nl; g) {
      ...
      foreach(n; nl) {
        ...
        foreach(k, v; preDp) {
          // k.deb;
          if (0 <= x && x < DR && k[x]) continue;

          auto tk = k[1..$].dup ~ false;
          if (1 <= x && x <= DR) tk[x - 1] = true;
          // deb(tk, v);
          auto tki = tk.idup;
          dp[tki] += v;
          dp[tki] %= MOD;
