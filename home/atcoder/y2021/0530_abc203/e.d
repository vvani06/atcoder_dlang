...
void problem() {
  auto N = scan!long;
  auto M = scan!long;
  auto P = M.iota.map!(x => GridPoint(scan!long, scan!long)).array;

  auto solve() {
    long[][long] blackMapRaw;
    foreach(p; P) {
      if (p.y in blackMapRaw) {
        blackMapRaw[p.y] ~= p.x;
      } else {
        blackMapRaw[p.y] = [p.x, 2*N + 1];
      }
    }
    SortedRange!(long[])[long] blackMap;
    foreach(k, v; blackMapRaw) blackMap[k] = v.sort();

    auto queue = DList!GridPoint(GridPoint(0, N));
    bool[long] goals;
    ...
    while(!queue.empty) {
      ...
      long nextX = 2 * N;
      if (p.y in blackMap) {
        auto upper = blackMap[p.y].upperBound(p.x);
        if (!upper.empty) nextX = upper[0] - 1;
      }
      if (nextX != p.x) queue.insert(GridPoint(nextX, p.y));

      auto up = p.up;
      if (up.y >= 0 && (up.y in blackMap)) {
        foreach(nx; blackMap[up.y].upperBound(up.x)) {
          if (nx > nextX + 1) break;
          queue.insert(GridPoint(nx, up.y));
        }
      }
      auto down = p.down;
      if (down.y <= 2*N && (down.y in blackMap)) {
        foreach(nx; blackMap[down.y].upperBound(down.x)) {
          if (nx > nextX + 1) break;
          queue.insert(GridPoint(nx, down.y));
        }
      }
    }
