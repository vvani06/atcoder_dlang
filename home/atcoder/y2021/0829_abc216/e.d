...
void problem() {
  auto N = scan!long;
  auto K = scan!long;
  auto A = scan!long(N).sort.reverse.array;
  ...
  auto solve() {
    ...
    foreach(a; A[1..$] ~ 0) {
      ...
      if (times + t > K) {
        const cycle = (K - times) / past;
        [cycle, past, K, times].deb;
        if (cycle > 0) {
          const b = pre - cycle;
          ans += past * ((pre + 1) * pre - (b + 1) * b) / 2;
        }
        [ans].deb;
        const rest = K - times - cycle * past;
        ans += (pre - cycle) * rest;
        break;
      } else {
        ans += past * ((pre + 1) * pre - (a + 1) * a) / 2;
        times += past * (pre - a);
      }
