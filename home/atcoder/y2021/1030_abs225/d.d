...
void problem() {
  ...
  class Query {
    ...
    this() {
      type = scan!long;
      x = scan!long - 1;
      if (type < 3) y = scan!long - 1;
    }
  }
  
  auto Q = QN.iota.map!(_ => new Query()).array;

  struct DL {
    long prev, value, next;
    this(long v) {
      value = prev = next = v;
    }
  }
  ...
  auto solve() {
    auto dls = N.iota.map!(i => DL(i)).array;
    foreach(q; Q) {
      if (q.type == 1) {
        dls[q.x].prev = q.y;
        dls[q.y].next = q.x;
        ...
        auto t = dls[q.x];
        while(t.next != t.value) t = dls[t.next];
        auto ans = new long[](0);
        for(t = t; t.prev != t.value; t = dls[t.prev]) {
          ans ~= t.value + 1;
        }
        ans ~= t.prev + 1;
        "%s %s".writefln(ans.length, ans.toAnswerString);
      }
    }
  }
