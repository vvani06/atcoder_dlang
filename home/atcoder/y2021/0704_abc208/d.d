...
void problem() {
  auto N = scan!long;
  auto M = scan!long;
  auto P = scan!long(3 * M).chunks(3);

  auto solve() {
    enum long MAX = 2L^^60;
    auto costs = new long[][](N, N);
    foreach(i; 0..N) costs[i][] = MAX;
    foreach(i; 0..N) costs[i][i] = 0;
    foreach(p; P) costs[p[0] - 1][p[1] - 1] = p[2];
    ...
    foreach(i; 0..N) {
      ...
      costs.each!(cs => ans += cs.filter!(c => c < MAX).sum);
    }
    return ans;
  }
