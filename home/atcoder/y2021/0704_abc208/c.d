...
void problem() {
  ...
  auto solve() {
    ...
    if (rest == 0) {
      foreach(i; 0..N) every.writeln;
    } else {
      auto sorted = N.iota.map!(i => [i, A[i]]).array.sort!"a[1] < b[1]";
      const border = sorted[rest][1];
      foreach(i; 0..N) {
        if (border <= A[i]) writeln(every); else writeln(every + 1);
      }
    }
  }
