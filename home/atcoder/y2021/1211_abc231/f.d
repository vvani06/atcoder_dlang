...
void problem() {
  auto N = scan!long;
  auto A = scan!long(N);
  auto B = scan!long(N).compress;

  auto solve() {
    auto sortedB = B.dup.sort;
    auto segtree = SegTree!"a + b"(new long[](N));
    auto AB = zip(A, B).array.multiSort!("a[0] > b[0]", "a[1] < b[1]");
    long ans;
    foreach(ab, n; AB.group) {
      const b = ab[1];

      const uppers = sortedB.upperBound(b - 1).length;
      const removed = segtree.sum(b, N);
      ans += n * (uppers - removed);
      segtree.update(b, segtree.get(b) + n);
    }
