...
void problem() {
  ...
  auto solve() {
    const freq = T * (1 + 100 / T);

    long[] cycle;
    long prev = 0;
    foreach(i; 1..101) {
      const taxed = i * (100 + T) / 100;
      if (prev + 1 != taxed) cycle ~= prev + 1;
    ...
    long ans = (K / cycle.length) * (cycle[$ - 1] + 1);
    const modCycle = K % cycle.length;
    ans += modCycle == 0 ? -1 : cycle[modCycle - 1];
    return ans;
  }
