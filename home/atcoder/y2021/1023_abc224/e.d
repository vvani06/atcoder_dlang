...
void problem() {
  auto H = scan!long;
  auto W = scan!long;
  alias Node = Tuple!(long, "y", long, "x", long, "num", long, "index");
  auto N = scan!long;
  auto NODES = N.iota.map!(i => Node(scan!long - 1, scan!long - 1, scan!long, i)).array;

  auto solve() {
    enum compExp = "a.num == b.num ? a.index < b.index : a.num < b.num";
    auto xs = H.iota.map!(_ => redBlackTree!compExp(new Node[](0))).array;
    auto ys = W.iota.map!(_ => redBlackTree!compExp(new Node[](0))).array;
    foreach(node; NODES) {
      xs[node.y].insert(node);
      ys[node.x].insert(node);
    }
    
    long[Node] memo;
    auto xMemo = new long[](H); xMemo[] = int.max;
    auto xMemoV = new long[](H); 
    auto yMemo = new long[](W); yMemo[] = int.max;
    auto yMemoV = new long[](W);

    auto comp = Node(0, 0, 0, int.max);
    auto compX = Node(0, 0, 0, int.max);
    auto compY = Node(0, 0, 0, int.max);
    
    long moves(Node node) {
      if (node in memo) return memo[node];

      const x = node.x;
      const y = node.y;
      comp.num = node.num;
      long ret = max(xMemoV[y], yMemoV[x]);

      long yMax;
      foreach(nex; ys[x].upperBound(comp)) {
        if (nex.num > yMemo[x]) break;
        yMax.chmax(1 + moves(nex));
      }
      ret.chmax(yMax);
      yMemoV[x].chmax(yMax);
      yMemo[x].chmin(node.num);

      long xMax;
      foreach(nex; xs[y].upperBound(comp)) {
        if (nex.num > xMemo[y]) break;
        xMax.chmax(1 + moves(nex));
      }
      ret.chmax(xMax);
      xMemoV[y].chmax(xMax);
      xMemo[y].chmin(node.num);
    ...
    auto ans = new long[](N);
    foreach(node; NODES.sort!"a.num > b.num") {
      ans[node.index] = moves(node);
    }
