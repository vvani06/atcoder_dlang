...
void problem() {
  auto N = scan!long;
  auto P = N.iota.map!(i => Point(scan!long, scan!long)).array;
 
  auto solve() {
    long[][long] yg;
    bool[long][long] xg;
    foreach(p; P) {
      if (p.y in yg) yg[p.y] ~= p.x; else yg[p.y] = [p.x];
      if (p.x in xg) xg[p.x][p.y] = true; else xg[p.x] = [p.y: true];
    }
    ...
    foreach(y, xs; yg) {
      ...
      foreach(xc; xs.combinations(2)) {
        const x1 = xc[0];
        const x2 = xc[1];
        foreach(yx1; xg[x1].keys) {
          if (yx1 <= y) continue;
