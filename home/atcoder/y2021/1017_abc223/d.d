...
void problem() {
  auto N = scan!long;
  auto M = scan!long;
  auto AB = scan!long(M * 2).map!"a - 1".array.chunks(2).array;
 
  auto solve() {
    auto graph = Graph(N).addUnidirectionalEdges(AB);
    auto ans = graph.topologicalSort;

    return ans.length == N ? ans.map!"a + 1".toAnswerString : "-1";
  }
