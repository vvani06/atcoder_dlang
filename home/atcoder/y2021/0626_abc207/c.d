...
void problem() {
  auto N = scan!long;
  auto T = scan!long(3 * N).chunks(3);

  auto solve() {
    auto RT = new real[][](0, 0);
    enum real eps = 1.0 / 256;
    foreach(t; T) {
      if (t[0] == 1) RT ~= [t[1].to!real, t[2].to!real];
      if (t[0] == 2) RT ~= [t[1].to!real, t[2].to!real - eps];
      if (t[0] == 3) RT ~= [t[1].to!real + eps, t[2].to!real];
      if (t[0] == 4) RT ~= [t[1].to!real + eps, t[2].to!real - eps];
    }

    long ans;
    foreach(i, a; RT[0..N-1]) {
      foreach(j, b; RT[i+1..$]) {
        real left = max(a[0], b[0]);
        real right = min(a[1], b[1]);
        if (right >= left) {
          ans++;
        }
      }
