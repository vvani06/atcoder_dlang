...
void problem() {
  ...
  auto solve() {
    ...
    auto pBase = ps[0];
    enum real SEIDO = 8192;

    foreach(i; 0..SEIDO) {
      auto QQ = Q.dup;
      auto theta = 2.0 * PI * i / SEIDO;
      foreach(ref q; QQ) {
      ...
      auto qs = QQ.multiSort!("a.x < b.x", "a.y < b.y");
      auto qBase = qs[0];
...
struct Vector2 {
  ...
  bool isNear(Vector2 other) { return approxEqual(x, other.x, eps, eps) && approxEqual(y, other.y, eps, eps); }
