...
void problem() {
  auto N = scan!long;
  alias Skill = Tuple!(long, "time", long[], "preSkills");

  Skill[] S;
  foreach(i; 0..N) {
    auto t = scan!long;
    auto k = scan!long;
    S ~= Skill(t, scan!long(k).map!"a - 1".array);
  }

  auto solve() {
    bool[long] listed;
    bool[long] visited;
    for(auto q = DList!long(N - 1); !q.empty; ) {
      auto p = q.front; q.removeFront;
      if (p in visited) continue;

      visited[p] = true;
      listed[p] = true;
      foreach(n; S[p].preSkills) {
        if (!(n in visited)) q.insertBack(n);
      }
    }

    return listed.keys.map!(s => S[s].time).sum;
  }
