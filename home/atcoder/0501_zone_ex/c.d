...
void problem() {
  ...
  auto solve() {
    long[] ans;
    long maxScore;
    foreach(comb; N.iota.array.combinations(3)) {
      bool[long] nodes;
      foreach(c; comb) {
        nodes[c] = true;
        foreach(p; P) {
          if (p[0] == c) nodes[p[1]] = true;
          if (p[1] == c) nodes[p[0]] = true;
        }
      }

      if (maxScore.chmax(nodes.length)) ans = comb.dup;
    }

    return ans.map!"a.to!string".joiner(" ");
  }
