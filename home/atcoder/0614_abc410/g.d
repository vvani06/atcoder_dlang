...
void problem() {
  ...
  auto solve() {
    enum LIM = 10^^6 * 2 + 1;
    auto factrial = new MInt9[](LIM);
    factrial[0] = MInt9(1);
    foreach(i; 1..LIM) factrial[i] = factrial[i - 1] * MInt9(i);
    
    MInt9 ans;

    ans += (factrial[A + B] / factrial[min(A, B)]) * (factrial[C + D] / factrial[min(C, D)]);
    ans += (factrial[B + C] / factrial[min(B, C)]);
