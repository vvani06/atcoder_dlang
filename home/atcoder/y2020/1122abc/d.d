...
void problem() {
  ...
  void solve() {
    ...
    real dfs(long a, long b, long c) {
      ...
      const sum = a + b + c;
      real ret = 0;
      ret += a.to!real / sum * (memoize!dfs(a+1, b, c) + 1);
      ret += b.to!real / sum * (memoize!dfs(a, b+1, c) + 1);
      ret += c.to!real / sum * (memoize!dfs(a, b, c+1) + 1);
      
      return ret;
    }

    writefln("%.10f", dfs(A, B, C));
  }
