...
void problem() {
  ...
  auto solve() {
    auto segTree = SegTree!("max(a, b)", long)(new long[](N), long.min);
    segTree.update(0, A[0]);

    foreach(i; 1..N) {
      auto best = segTree.sum(max(0, i - K), i);
      segTree.update(i, best + A[i]);
    }

    return segTree.get(N - 1);
  }
