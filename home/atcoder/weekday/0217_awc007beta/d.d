...
void problem() {
  ...
  auto A = scan!int;
  auto B = scan!int;
  auto RCA = scan!int(4 * A).chunks(4);
  auto RCB = scan!int(4 * B).chunks(4);

  auto solve() {
    bool[] a = new bool[](N ^^ 2);
    foreach(rs, cs, re, ce; RCA.asTuples!4) {
      foreach(r; rs - 1..re) a[r*N + cs - 1 .. r*N + ce] = true;
    }
    bool[] b = new bool[](N ^^ 2);
    foreach(rs, cs, re, ce; RCB.asTuples!4) {
      foreach(r; rs - 1..re) b[r*N + cs - 1 .. r*N + ce] = true;
    }

    return iota(N^^2).count!(i => a[i] && b[i]);
  }
