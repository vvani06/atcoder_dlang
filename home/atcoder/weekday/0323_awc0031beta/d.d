...
void problem() {
  ...
  auto solve() {
    auto bookHeap = R.heapify!"a > b";
    auto removeHeap = new int[](0).heapify!"a > b";

    int cur;
    foreach(d, l; L.sort.array.enumerate(0)) {
      while(cur < l && !bookHeap.empty) {
        removeHeap.insert(d + bookHeap.front - 1);
        bookHeap.removeFront();
        cur++;
      }

      while(!removeHeap.empty && removeHeap.front <= d) {
        removeHeap.removeFront();
        cur--;
      }
    }
    
    return bookHeap.empty && removeHeap.empty; 
  }
