...
void problem() {
  auto N = scan!int;
  auto P = scan!long(N);
  auto W = scan!long(N - 1);

  auto solve() {
    long[][] memo = new long[][](2^^N, N);
    foreach(fromState; 1..2^^N) {
      foreach(from; 0..N) {
        if ((2^^from & fromState) == 0) continue;

        foreach(add; 0..N) {
          auto toState = fromState | (2^^add);
          if (fromState == toState) continue;

          auto wi = fromState.popcnt - 1;
          auto w = W[wi];

          memo[toState][add].chmax(memo[fromState][from] + abs(P[from] - P[add]) * w);
        }
      }
    }

    return memo.back.maxElement;
  }
