...
void problem() {
  ...
  auto M = scan!int;
  auto V = scan!long(N);
  auto DT = scan!long(2 * M).chunks(2).array;

  auto solve() {
    auto heap = V.heapify!"a > b";
    int ans;
    foreach(d, t; DT.sort!"a[0].to!real / a[1].to!real < b[0].to!real / b[1].to!real".asTuples!2) {
      while(!heap.empty && heap.front * t < d) heap.removeFront();
