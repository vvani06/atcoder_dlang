...
void problem() {
  ...
  auto M = scan!int;
  auto T = scan!int(N);
  auto UV = scan!int(2 * M).chunks(2).array;
  ...
  auto solve() {
    ...
    auto ans = new bool[](N);
    for(auto queue = DList!int(iota(N).filter!(i => degrees[i] > T[i])); !queue.empty;) {
      ...
      foreach(next; graph[cur]) {
        ...
        if (++degrees[next] == T[next] + 1) queue.insert(next);
      }
    }

    auto tAns = iota(1, N + 1).filter!(i => ans[i - 1]).array;
    return tAns.empty ? [-1] : tAns;
  }
