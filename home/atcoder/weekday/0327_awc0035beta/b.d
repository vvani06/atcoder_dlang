...
void problem() {
  ...
  auto solve() {
    ...
    long ans;
    foreach(d; D) {
      if (d in sorted) continue;

      auto left = sorted.lowerBound(d);
      auto right = sorted.upperBound(d);
      
      long best = long.max;
      if (!left.empty) best = min(best, d - left.back);
      if (!right.empty) best = min(best, right.front - d);
      ans += best;
    }
    return ans;
  }
