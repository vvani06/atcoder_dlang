...
void problem() {
  ...
  auto solve() {
    ...
    Edge[][] graph = new Edge[][](N + 1, 0);
    foreach(a, b, c; ABC.asTuples!3) {
      graph[a] ~= Edge(b.to!int, c);
      graph[b] ~= Edge(a.to!int, c);
    }

    long[] memo = new long[](N + 1);
    memo[] = INF;
    memo[1] = 0;
    for(auto queue = [Edge(1, 0)].heapify!"a.cost > b.cost"; !queue.empty; ) {
      auto cur = queue.front;
      queue.removeFront();
      if (memo[cur.node] != cur.cost) continue;

      foreach(next; graph[cur.node]) {
        auto to = next.node;
        auto cost = cur.cost + next.cost;
        if (memo[to].chmin(cost)) {
          queue.insert(Edge(to, cost));
        }
      }
    }

    return memo[T] == INF ? -1 : memo[T] * 2;
  }
