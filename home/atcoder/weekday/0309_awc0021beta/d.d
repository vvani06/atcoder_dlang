...
void problem() {
  ...
  auto M = scan!int;
  auto K = scan!int;
  auto E = iota(M).map!(_ => tuple(scan!int, scan!int, scan!long)).array;
  auto P = scan!int(K);
  enum INF = long.max / 30;

  auto solve() {
    alias Edge = Tuple!(int, "node", long, "cost");
    auto graph = new Edge[][](N + 1, 0);
    foreach(u, v, t; E.asTuples!3) {
      graph[u] ~= Edge(v, t);
      graph[v] ~= Edge(u, t);
    }

    long[] calcCosts(int from) {
      auto ret = INF.repeat(N + 1).array;
      ret[from] = 0;
      for(auto queue = [Edge(from, 0)].heapify!"a.cost > b.cost"; !queue.empty;) {
        auto cur = queue.front;
        queue.removeFront();
        if (cur.cost != ret[cur.node]) continue;

        foreach(next; graph[cur.node]) {
          if (ret[next.node].chmin(cur.cost + next.cost)) queue.insert(Edge(next.node, cur.cost + next.cost));
        }
      }
    ...
    // route.deb;
    long ans;
    foreach(from, to; iota(K + 1).map!(i => [route[i], route[i + 1]]).asTuples!2) {
      ans += calcCosts(from)[to];
    }
