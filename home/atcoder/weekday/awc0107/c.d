...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto SV = iota(N).map!(_ => tuple(_, scan!string, scan!int)).array;
  auto X = scan!int(Q);

  auto solve() {
    string[][int] dict;
    int[int] minId;
    foreach(i, s, v; SV.asTuples!3) {
      dict[v] ~= s;
      if (!(v in minId)) minId[v] = i;
    }

    auto vs = SV.map!"a[2]".array.sort;
    foreach(x; X) {
      if (x in dict) {
        writeln(dict[x].joiner(" "));
      } else {
        auto lc = vs.lowerBound(x);
        auto rc = vs.upperBound(x);
        auto l = lc.empty ? int.min : lc.back;
        auto r = rc.empty ? int.max : rc.front;
        auto ld = abs(l - x);
        auto rd = abs(r - x);
        auto ans = [[ld, minId.get(l, -1), l], [rd, minId.get(r, -1), r]].minElement[2];
        writeln(dict[ans][0]);
      }
    }
  }
