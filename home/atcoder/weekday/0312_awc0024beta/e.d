...
void problem() {
  ...
  auto solve() {
    ...
    foreach(l, c; LC.asTuples!2) {
      ...
      foreach(bn; units ~ rest) {

        rest -= bn;
        foreach_reverse(from; 0..W) {
          if (memo[from] == INF) continue;

          auto to = from + l * bn;
          if (to > W) continue;

          memo[to] = min(memo[to], memo[from] + bn);
        }
      }
    }
