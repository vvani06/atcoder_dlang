...
void problem() {
  ...
  auto M = scan!int;
  auto K = scan!int;
  auto AB = scan!int(2 * M).chunks(2);

  auto solve() {
    auto uf = UnionFind(N + 1);
    foreach(k; iota(1, K + 1)) uf.unite(1, k);
    foreach(a, b; AB.asTuples!2) {
      if (uf.root(a) == uf.root(1) || uf.root(b) == uf.root(1)) uf.unite(a, b);
    }

    return iota(1, N + 1).count!(i => uf.root(i) == uf.root(1));
  }
