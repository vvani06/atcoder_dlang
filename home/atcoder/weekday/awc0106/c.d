...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto XY = [0, 0] ~ scan!long(2 * N).chunks(2).array;
  ...
  auto solve() {
    ...
    long bfs(int from, int to) {
      auto ret = new long[](N + 1);
      ret[] = -1;
      ret[from] = 0;
      ...
      for(auto queue = DList!int(from); !queue.empty;) {
        ...
        foreach(next; graph[cur]) {
          ...
          ret[next] = ret[cur] + dist(cur, next);
