...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto AB = [0L, 0L] ~ scan!long(2 * N).chunks(2).array ~ [0L, 0L];
  auto X = scan!long(Q);
  ...
  auto solve() {
    struct UnionFindExtra { 
      ...
      UnionFindExtra merge(UnionFindExtra other) { return UnionFindExtra(max(score, other.score)); }
    }
    alias UnionFind = UnionFindWith!UnionFindExtra;

    auto uf = UnionFind(N + 2);
    foreach(i, b; zip(iota(N + 2), AB.map!"a[1]")) uf.setExtra(i, UnionFindExtra(b));

    auto mounts = AB.map!"a[0]".enumerate(0).array.heapify!"a[1] < b[1]";
    auto revealed = new bool[](N + 2);

    auto ans = new long[](Q);
    long sum;
    foreach(qi, x; X.enumerate(0).array.sort!"a[1] > b[1]".asTuples!2) {
      while(!mounts.empty && mounts.front[1] >= x) {
        auto mount = mounts.front;
        mounts.removeFront();
        revealed[mount[0]] = true;
        sum += uf.extra(mount[0]).score;

        foreach(next; [mount[0] - 1, mount[0] + 1]) {
          if (revealed[next] && !uf.same(next, mount[0])) {
            sum -= uf.extra(mount[0]).score;
            sum -= uf.extra(next).score;
            uf.unite(next, mount[0]);
            sum += uf.extra(next).score;
          }
        }
      }
