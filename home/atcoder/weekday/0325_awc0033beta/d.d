...
void problem() {
  ...
  auto M = scan!int;
  auto K = scan!int;
  auto UVC = iota(M).map!(i => tuple(scan!int, scan!int, scan!long)).array;

  auto solve() {
    alias Cursor = Tuple!(int, "node", long, "cost");
    auto graph = new Cursor[][](N + 1, 0);
    foreach(u, v, c; UVC.asTuples!3) {
      graph[u] ~= Cursor(v, c);
      graph[v] ~= Cursor(u, c);
    }

    enum INF = long.max / 3;
    auto costs = INF.repeat(N + 1).array;
    costs[1] = 0;
    for(auto queue = [Cursor(1, 0)].heapify!"a.cost > b.cost"; !queue.empty;) {
      auto cur = queue.front;
      queue.removeFront();
      if (costs[cur.node] != cur.cost) continue;

      foreach(edge; graph[cur.node]) {
        if (costs[edge.node].chmin(cur.cost + edge.cost)) queue.insert(Cursor(edge.node, cur.cost + edge.cost));
      }
    }

    return costs[N] <= K ? costs[N] : -1;
  }
