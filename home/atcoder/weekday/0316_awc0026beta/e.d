...
void problem() {
  ...
  auto solve() {
    auto maxSegTree = SegTree!("max(a, b)", long)(A, int.max);
    auto minSegTree = SegTree!("min(a, b)", long)(A, int.min);

    auto isLower(int l, long k) {
      return (int x) => maxSegTree.sum(l, x) - minSegTree.sum(l, x) < k;
    }
    
    long ans;
    foreach(l; 0..N) {
      auto r1 = binarySearch(isLower(l, K), l, N + 1);
      auto r2 = binarySearch(isLower(l, K + 1), l, N + 1);
      ans += r2 - r1;
    }
    return ans;
  }
