...
void problem() {
  auto N = scan!int;
  auto M = scan!int;
  auto FD = scan!int(2 * N).chunks(2);

  auto solve() {
    struct Fruit {
      long amount, decay;

      inout int opCmp(inout Fruit other) {
        return cmp(
          [amount, -decay],
          [other.amount, -other.decay],
        );
      }

      Fruit next() {
        return Fruit(max(0, amount - decay), decay);
      }
    }

    long ans;
    int m = M;
    for(auto queue = FD.map!(fd => Fruit(fd[0], fd[1])).array.heapify; m > 0; m--) {
