...
void problem() {
  ...
  auto solve() {
    auto maxTree = SegTree!("max(a, b)", long)(H);
    auto minTree = SegTree!("min(a, b)", long)(H);

    int nearestLarger(int from) {
      bool isOk(int i) {
        auto mx = maxTree.sum(from, i);
        return mx > H[from];
      }

      return binarySearch(&isOk, N + 1, from);
    }

    long ans;
    foreach(i; 0..N) {
      auto catcher = nearestLarger(i);
      if (catcher <= N) {
        ans += minTree.sum(i, catcher);
      }
    }
