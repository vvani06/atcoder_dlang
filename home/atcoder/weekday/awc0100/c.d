...
void problem() {
  ...
  auto M = iota(Q).map!(_ => scan!int(scan!int)).array;

  auto solve() {
    auto bitArrays = S.map!(s => BitArray(s.map!"a == '1'".array)).array;

    foreach(m; M) {
      auto ans = m.map!(i => bitArrays[i - 1]).fold!"a | b";
      writeln(ans.array.map!"a ? '1' : '0'".to!string);
    }
  }
