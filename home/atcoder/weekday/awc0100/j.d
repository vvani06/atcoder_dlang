...
void problem() {
  auto N = scan!int;
  auto PW = scan!int(2 * N - 2).chunks(2).array;
  auto Q = scan!int;
  auto EQ = scan!int(2 * Q).chunks(2).array;
  auto R = scan!int;
  auto ER = scan!int(2 * R).chunks(2).array;

  auto solve() {
    alias Edge = Tuple!(int, "id", int, "node");
    auto graph = new Edge[][](N, 0);
    foreach(i, p; zip(iota(1, N), PW.map!"a[0]")) {
      graph[p - 1] ~= Edge(i - 1, i);
      graph[i] ~= Edge(i - 1, p - 1);
    }

    auto ins = new int[](N);
    auto outs = new int[](N);
    auto route = new int[](0);
    int rid;

    void dfs(int cur, int pre) {
      ins[cur] = rid;
      foreach(edge; graph[cur]) {
        if (edge.node == pre) continue;
    ...
    auto powers = route.map!(r => PW[r][1]).array;
    int[] adds = new int[](powers.length + 1);
    // int[] subs = new int[](powers.length);
    foreach(u, v; EQ.asTuples!2) {
      u--; v--;
      auto l = min(ins[u], ins[v]);
      auto r = max(ins[u], ins[v]);
      adds[l]++;
      adds[r]--;
    }

    int cur;
    foreach(i; 0..powers.length.to!int) {
      cur += adds[i];
      powers[i] += cur;
    }

    powers.deb;
    auto segTree = SegTree!("min(a, b)", int)(powers);
    foreach(u, v; ER.asTuples!2) {
      u--; v--;
      auto l = min(ins[u], ins[v]);
      auto r = max(ins[u], ins[v]);
      writeln(segTree.sum(l, r));
    }
  }
