...
void problem() {
  ...
  auto solve() {
    auto segTree = SegTree!("a + b", int)(1.repeat(N).array);

    int realIndex(int left, int index) {
      bool isOk(int i) {
        // [[[left, index, i, segTree.sum(left, i)]]].deb;
        return index > segTree.sum(left, i);
      }

      return binarySearch(&isOk, left, N + 1);
    }

    auto cur = S - 1;
    foreach(i, d; D) {
      segTree.update(cur, 0);
      // segTree.array.deb;

      auto r = segTree.sum(cur, N);
      // [cur, d, r].deb;
      ...
      if (d > r) {
        ...
        d %= (N - i - 1);
        if (d == 0) d = N - i - 1;
        cur = realIndex(0, d.to!int);
      } else {
        cur = realIndex(cur, d.to!int);
      }
    }
