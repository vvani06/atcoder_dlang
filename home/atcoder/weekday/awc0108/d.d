...
void problem() {
  auto N = scan!int;
  auto M = scan!int;
  auto C = scan!int(2 * N).chunks(2).map!"a[1]".array;
  auto UVK = scan!int(3 * M).chunks(3).array;

  auto solve() {
    alias Edge = Tuple!(int, "to", int, "key");
    auto graph = new Edge[][](N, 0);
    foreach(u, v, k; UVK.asTuples!3) {
      graph [u - 1] ~= Edge(v - 1, k);
    }
    
    auto obtained = new bool[](N + 1);
    obtained[0] = true;
    int[][int] waiting;
    ...
    for(auto queue = DList!int(0); !queue.empty;) {
      ...
      if (!obtained[C[node]]) {
        obtained[C[node]] = true;
        foreach(opened; waiting.get(C[node], [])) {
          queue.insertBack(opened);
        }
      }

      foreach(edge; graph[node]) {
        if (visited[edge.to]) continue;

        if (obtained[edge.key]) {
          queue.insertBack(edge.to);
        } else {
          waiting[edge.key] ~= edge.to;
        }
      }
    }
