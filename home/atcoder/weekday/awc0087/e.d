...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto SP = iota(N).map!(_ => tuple(scan!string, scan!long)).array;
  auto LR = scan!int(2 * Q).chunks(2).array;

  auto solve() {
    struct FlipValue {
      long ans, left, right, all;

      FlipValue add(FlipValue r) {
        auto l = this;
        return FlipValue(
          max(l.ans, r.ans, l.right + r.left),
          max(l.left, l.all + r.left),
          max(l.right + r.all, r.right),
          l.all + r.all
        );
      }
    }

    auto acc = 0L ~ iota(N).map!(i => SP[i][0] == "A" ? SP[i][1] : 0).cumulativeFold!"a + b".array;
    auto deltas = iota(N).map!(i => SP[i][0] == "A" ? SP[i][1] : -SP[i][1]).array;

    auto segTree = SegTree!("a.add(b)", FlipValue)(deltas.map!(x => FlipValue(max(0L, x), max(0L, x), max(0L, x), x)).array, FlipValue(0, 0, 0, 0));

    foreach(l, r; LR.asTuples!2) {
      writeln(acc[r] - acc[l - 1] - segTree.sum(l - 1, r).ans);
    }
  }
