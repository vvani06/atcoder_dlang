...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto YP = scan!long(2 * N).chunks(2).array;
  auto L = scan!long(Q);

  auto solve() {
    auto comp = CompressTable!long(YP.map!"a[0]".array ~ L);
    long[] v = new long[](N + Q + 1);
    foreach(y, p; YP.asTuples!2) v[comp[y]] += p;

    auto acc = 0L ~ v.cumulativeFold!"a + b".array;
    return L.map!(l => acc.back - acc[comp[l]]);
  }
