...
void problem() {
  ...
  auto solve() {
    auto nsq = N.to!real.sqrt.to!int;
    auto segTree = SegTree!("a + b", long)(0L ~ S);
    ...
    foreach(_; 0..Q) {
      ...
      if (T == 1) {
        ...
        if (K <= nsq) {
          ...
          foreach(n; iota(K, N + 1, K)) segTree.add(n, V);
        }
      } else {
        auto X = scan!int;
        long ret = segTree.sum(0, X + 1);
        foreach(n; 1..nsq + 1) ret += bonus[n] * (X / n);
        ans ~= ret;
      }
    }
