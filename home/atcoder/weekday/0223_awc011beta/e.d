...
void problem() {
  auto N = scan!int;
  auto M = scan!int;
  auto AB = scan!long(2 * N).chunks(2);
  auto A = AB.map!"a[0]".array;
  auto B = AB.map!"a[1]".array;

  auto solve() {
    enum MIN = long.min / 2;
    auto memo = new long[][](N + 1, M + 1);
    foreach(ref m; memo) m[] = MIN;
    memo[0][0] = 0;

    foreach(i, a, b; zip(iota(N), A, B)) {
      foreach(from; 0..M + 1) {
        if (memo[i][from] == MIN) continue;

        memo[i + 1][from].chmax(memo[i][from]);
        if (from + a <= M) {
          memo[i + 1][from + a].chmax(memo[i][from] + b);
        }
      }
    }

    auto best = memo.map!maxElement.maxElement;
    auto reachable = new bool[][](N + 1, M + 1);
    foreach(i; 0..N + 1) foreach(j; 0..M + 1) reachable[i][j] = memo[i][j] == best;
    ...
    foreach(i; iota(1, N + 1).retro) foreach(j; 0..M + 1) {
      ...
      if (j >= A[i - 1] && memo[i][j] - B[i - 1] == memo[i - 1][j - A[i - 1]]) {
        reachable[i - 1][j - A[i - 1]] = true;
        use[i - 1] = true;
      }
    }
