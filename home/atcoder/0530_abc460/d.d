...
void problem() {
  ...
  auto solve() {
    auto graph = GridGraph!(AroundDelta.RC8)(H, W);

    auto grid = S.map!"a.array".joiner.array;
    auto dists = new int[](H * W);
    dists[] = BIG;
    
    auto blacks = DList!int(new int[](0));
    foreach(from; graph.nodes) {
      if (grid[from] == '#') continue;

      foreach(next; graph.nexts(from)) {
        if (grid[next] == '#') {
          blacks.insertBack(next);
          dists[next] = 1;
        }
      }
    }
    ...
    while(!blacks.empty) {
      auto cur = blacks.front;
      blacks.removeFront();

      foreach(next; graph.nexts(cur)) {
        if (grid[next] == '.' || dists[next] != BIG) continue;

        blacks.insertBack(cur);
        dists[next] = dists[cur] + 1;
      }
    }
    foreach(node; graph.nodes) {
      if (grid[node] == '#' && (dists[node] == BIG || dists[node] % 2 == 0)) grid[node] = '.';
    }
    
    auto whites = DList!int(new int[](0));
    foreach(node; graph.nodes) {
      if (grid[node] == '.') continue;

      foreach(next; graph.nexts(node)) {
        if (grid[next] == '#') continue;

        whites.insertBack(next);
        dists[next] = 1;
      }
    }

    while(!whites.empty) {
      auto cur = whites.front;
      whites.removeFront();
