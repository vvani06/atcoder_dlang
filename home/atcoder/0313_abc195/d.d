...
void problem() {
  ...
  auto subSolve(long[][] bags, long[] boxes) {
    bool[] used = new bool[](boxes.length);

    auto sortedBoxes = boxes.sort().array;
    sortedBoxes.deb;
    long ans;
    foreach(b; bags.sort!"a[1] > b[1]") {
      foreach(i, x; sortedBoxes) {
        if (!used[i] && x >= b[0]) {
          used[i] = true;
          ans += b[1];
          break;
  ...
  auto solve() {
    foreach(q; Queries) {
      q.deb;
      subSolve(Baggages, Boxes[0..q[0] - 1] ~ Boxes[q[1]..$]).writeln;
    }
  }
