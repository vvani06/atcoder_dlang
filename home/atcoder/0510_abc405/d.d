...
void problem() {
  ...
  auto solve() {
    auto ans = S.map!array.array;

    alias Coord = Tuple!(int, "r", int, "c");
    auto queue = DList!Coord();
    foreach(r; 0..H) foreach(c; 0..W) {
      if (S[r][c] == 'E') queue.insertBack(Coord(r, c));
    }
    ...
    while(!queue.empty) {
      ...
      foreach(dr, dc, move; zip([0, -1, 0, 1], [-1, 0, 1, 0], ">v<^")) {
        auto r = cur.r + dr;
        auto c = cur.c + dc;
    ...
    foreach(a; ans) writeln(a.to!string);
  }
