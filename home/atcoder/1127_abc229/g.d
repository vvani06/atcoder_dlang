...
void problem() {
  ...
  auto solve() {
    long[] ys;
    foreach(i, c; S) if (c == 'Y') ys ~= i;
    long[] steps;
    foreach(i, y; ys) steps ~= y - i;
    long[] acc = [0];
    foreach(s; steps) acc ~= acc[$ - 1] + s;

    long ans;
    foreach(long left; 0..steps.length) {
      bool groupable(long right) {
        if (right > steps.length) return false;

        auto arr = steps[left..right];
        auto arc = acc[left..right + 1];
        ...
        // [mid, arr[mid]].deb;
        // [arr[0..mid], arr[mid..$]].deb;
        // [arc[mid] - arc[0], arc[$ - 1] - arc[mid]].deb;
        // [arr[mid], arr[mid] * mid].deb;

        const mids = arr[mid] * mid;
        const need = (mids - arc[mid] + arc[0]) + (arc[$ - 1] - arc[mid] - arr[mid]*((right - left) - mid));
        const ret = need <= K;

        // deb([left, right], right - left, " : ", need, " : ", ret);
        return ret;
      }

      ans = max(ans, binarySearch(&groupable, left, steps.length + 1) - left);
    }
