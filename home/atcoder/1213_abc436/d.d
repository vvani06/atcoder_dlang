...
void problem() {
  ...
  auto solve() {
    auto teleports = new Coord[][](256, 0).map!(t => t.redBlackTree).array;
    foreach(r; 0..H) foreach(c; 0..W) {
      auto g = G[r][c];
      if (g >= 'a' && g <= 'z') teleports[g].insert(Coord(r, c));
    }

    int[][] ans = new int[][](H, W);
    foreach(ref a; ans) a[] = INF;
    ans[0][0] = 0;
    ...
    for(auto queue = DList!Coord([Coord(0, 0)]); !queue.empty; ) {
      ...
      auto g = G[cur.r][cur.c];
      if (g >= 'a' && g <= 'z') {
        foreach(t; teleports[g]) {
          if (ans[t.r][t.c] != INF) continue;

          ans[t.r][t.c] = ans[cur.r][cur.c] + 1;
          queue.insertBack(t);
        }
        teleports[g].clear;
      }
      ...
      foreach(dr, dc; zip([-1, 0, 1, 0], [0, -1, 0, 1])) {
        ...
        if (G[r][c] == '#' || ans[r][c] != INF) continue;

        ans[r][c] = ans[cur.r][cur.c] + 1;
        queue.insertBack(Coord(r, c));
      }
    }

    return ans[H - 1][W - 1] == INF ? -1 : ans[H - 1][W - 1];
  }
