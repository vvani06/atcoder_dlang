...
void problem() {
  ...
  auto subSolve(int N, int[][] E) {
    ...
    int[] restDeg = degrees.dup;
    for(auto queue = DList!int(iota(N).filter!(i => degrees[i] <= 1).array); !queue.empty;) {
      auto cur = queue.front;
      queue.removeFront();
      if (memo[cur] != -1) continue;
    ...
    return max(ans, memo.maxElement);
  }
