...
void problem() {
  ...
  auto solve() {
    string[] patterns;
    patterns ~= ('.'.repeat(B).array ~ '#'.repeat(B).array).to!string;
    patterns ~= ('#'.repeat(B).array ~ '.'.repeat(B).array).to!string;
    patterns[0] = patterns[0].repeat(10).joiner.array.to!string;
    patterns[1] = patterns[1].repeat(10).joiner.array.to!string;

    string[] ans;
    foreach(y; 0..N) {
      foreach(t; 0..A) {
        ans ~= patterns[y % 2][0..N * B];
      }
    }
