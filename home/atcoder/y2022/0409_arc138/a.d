...
void problem() {
  auto N = scan!long;
  auto K = scan!long;
  auto A = scan!long(N).compress;
  long INF = int.max;

  auto solve() {
    const S = A[0..K].sum;
    auto topSum = A.dup.sort.reverse.array[0..K].sum;
    if (topSum <= S) return -1;

    auto segtree = SegTree!("min(a, b)", long)(INF.repeat(N).array, INF);
