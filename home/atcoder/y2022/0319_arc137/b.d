...
void problem() {
  ...
  auto solve() {
    auto g = A.group.map!(a => a[0] == 0 ? -cast(int)a[1] : cast(int)a[1]).array;
    g.deb;
    
    auto acc = new int[](g.length + 1);
    int ma, macc;
    int mi, micc;
    foreach(i, a; g) {
      acc[i + 1] = acc[i] + a;
      macc = max(macc, acc[i + 1]);
      ma = max(ma, macc - acc[i + 1]);
      micc = min(micc, acc[i + 1]);
      mi = min(mi, micc - acc[i + 1]);
    }
