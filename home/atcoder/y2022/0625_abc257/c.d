...
void problem() {
  ...
  auto solve() {
    long[] children, adalts;
    foreach(i; 0..N) if (S[i] == '0') children ~= W[i]; else adalts ~= W[i];
    auto cr = children.sort;
    auto ar = adalts.sort;

    auto calc = (long w) => [w, w + 1].map!(x => cr.lowerBound(x).length + ar.upperBound(x - 1).length).maxElement;
    return W.map!(w => calc(w)).maxElement;
  }
