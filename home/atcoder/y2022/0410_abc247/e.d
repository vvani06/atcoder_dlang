...
void problem() {
  ...
  auto solve() {
    long[][] genSubArrays() {
      ...
      if (maxed && mined) subArrays ~= A[tl..$];
      return subArrays;
    }

    long ans;
    foreach(arr; genSubArrays()) {
      long[] maxss, minss;
      foreach(i, a; arr) {
        if (a == X) maxss ~= i;
        if (a == Y) minss ~= i;
      ...
      auto maxs = maxss.assumeSorted;
      auto mins = minss.assumeSorted;
      auto l = arr.length;
      // [maxs, mins].deb;
      foreach(long i, a; arr) {
        auto maxIndicies = maxs.upperBound(i - 1);
        auto minIndicies = mins.upperBound(i - 1);
        if (maxIndicies.empty || minIndicies.empty) break;

        ans += l - max(maxIndicies.front, minIndicies.front);
        (l - max(maxIndicies.front, minIndicies.front)).deb;
      }
    }
