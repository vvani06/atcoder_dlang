...
void problem() {
  ...
  auto solve() {
    auto subSolve(int N, string s) {
      int[] pls;
      auto dfs(string t) {
        auto tn = t.length.to!int;
        auto l = (tn - 1) / 2;
        auto r = tn / 2;

        bool[char] cnt;
        while(l >= 0 && r < tn) {
          if (t[l] != t[r]) return tn;
          cnt[t[l]] = cnt[t[r]] = true;
          l--; r++;
        }
        ...
        return tn % 2 == 0 ? dfs(t[0..tn / 2]) : tn;
      }

      dfs(s);
      foreach_reverse(pl; pls) {
        auto pq = s ~ s[0..pl];
        auto pql = pq.length.to!int;
        auto l = (pql - 1) / 2;
        auto r = pql / 2;
        bool isNg;
        while(l >= 0 && r < pql) {
    ...
    foreach(i; 0..T) {
      writefln("Case #%s: %s", i + 1, subSolve(scan!int, scan));
    }
  }
