...
void problem() {
  ...
  auto solve() {
    auto counts = 27.iota.map!(i => MInt9(i) ^^ L).array;
    auto bitSets = S.map!(s => s.map!(c => 2^^(c - 'a')).sum).array;
    auto memo = new int[](2^^N);
    memo[0] = ALL;
    ...
    foreach(n; 1..2^^N) {
      const add = n.bsr;
      const pre = n ^ (2^^add);
      memo[n] = memo[pre] & bitSets[add];

      auto count = counts[popcnt(memo[n])];
      if (popcnt(n) % 2 == 1) ans += count; else ans -= count;
    }
