...
void problem() {
  ...
  auto solve() {
    alias Edge = Tuple!(int, "id", int, "f", int, "t", long, "cost");
    auto g = new Edge[][](N, 0);
    foreach(int i, e; E) {
      e[0]--; e[1]--;
      g[e[0]] ~= Edge(i + 1, e[0], e[1], e[2]);
      g[e[1]] ~= Edge(i + 1, e[1], e[0], e[2]);
    }
    ...
    for(auto q = ([TRY(Edge(0, 0, 0, 0), 0)]).heapify!"a.total > b.total"; !q.empty;) {
      ...
      foreach(next; g[e.t]) {
        ...
        auto c = p.total + next.cost;
        if (costs[next.t].chmin(c)) {
          q.insert(TRY(next, c));
        }
      }
    }

    return ans[1..$].toAnswerString;
  }
