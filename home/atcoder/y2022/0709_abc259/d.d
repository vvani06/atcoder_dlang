...
void problem() {
  ...
  auto solve() {
    ...
    foreach(i; 0..N - 1) foreach(j; i + 1..N) {
      auto ma = (R[i] + R[j])^^2;
      auto mi = (R[i] - R[j])^^2;
      auto norm = V[i].norm(V[j]);
      if (norm < mi || norm > ma) continue;

      graph[i] ~= j;
      graph[j] ~= i;
    }
    
    auto goals = N.iota.map!(i => V[i].norm(T) == R[i]^^2).array;
    int[] starts;
    foreach(i; 0..N) {
      if (V[i].norm(S) == R[i]^^2) starts ~= i;
    }
    ...
    for(auto q = DList!int(starts); !q.empty;) {
      auto p = q.front;
      q.removeFront;
      if (goals[p]) return YESNO[true];
      if (visited[p]) continue;
      visited[p] = added[p] = true;
