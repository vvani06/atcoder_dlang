...
void problem() {
  ...
  enum ulong SUMN = 10 ^^ 6;

  long[size + 1] genPow(ulong base, ulong size, ulong MOD)() {
    long[size + 1] ret;
    ret[0] = 1;
    foreach(i; 0..size) ret[i + 1] = (ret[i] * base) % MOD;
    return ret;
  }
  enum POW26 = genPow!(26UL, SUMN, MOD);

  auto subSolve(long N, char[] S) {
    long mid = N / 2 + N % 2;
    ...
    foreach(i; 0..mid) {
      long lower = S[i] - 'A';
      long rest = mid - i - 1;
      // [i, mid, rest, lower].deb;
      ans += lower * POW26[rest];
      ans %= MOD;
    }

    if (S[0..mid].dup.reverse.array.to!string <= S[$ - mid..$]) ans++;
  ...
  auto solve() {
    foreach(_; 0..T) {
      subSolve(scan!long, cast(char[])scan).writeln;
    }
  }
