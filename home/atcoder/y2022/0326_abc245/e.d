...
void problem() {
  ...
  auto solve() {
    long[long] compX;
    long pre, same;
    foreach(i, x; (A ~ C).sort.array) {
      if (pre == x) same++;
      compX[x] = i - same;
      pre = x;
    }

    auto Y = (B ~ D).sort.array;
    long[long] compY;
    pre = same = 0;
    foreach(i, x; Y) {
      if (pre == x) same++;
    ...
    auto ys = new long[][](N + M, 0);
    long[long] xs;
    foreach(i, c; C) {
      ys[c] ~= D[i];
      xs[c]++;
    }
    auto xt = xs.keys.redBlackTree;
    auto yt = ys.map!(y => y.redBlackTree!true);

    foreach(i; 0..N) {
      const a = A[i];
      const b = B[i];

      foreach(x; xt.upperBound(a - 1)) {
        auto y = yt[x].upperBound(b - 1);
        // deb([a, b, x], y);
        if (y.empty) return YESNO[false];

        // [[a, b], [x, y.front]].deb;
        yt[x].removeKey(y.front);
        if (yt[x].empty) xt.removeKey(x);
        break;
      }
    }
