...
void problem() {
  ...
  auto solve() {
    ...
    foreach(b; 0..60) {
      long bn = 2L ^^ b;
      if ((bn & M) == bn) maxBitInM = b + 1;
    }

    MInt9 numsInMaxBit(long maxBit) {
      if (maxBit > maxBitInM) return MInt9(0);
      if (maxBit < maxBitInM) return MInt9(2) ^^ (maxBit - 1);

      return MInt9(M + 1) - MInt9(2)^^(maxBitInM - 1);
    }

    auto dp = new MInt9[](61);
    auto pre = new MInt9[](61);
    dp[0] = MInt9(1);
    foreach(i; 0..N) {
      swap(dp, pre);
      dp = new MInt9[](61);
      foreach(from; 0..60) foreach(to; from+1..61) {
        dp[to] += pre[from] * memoize!numsInMaxBit(to);
      }
      
      if (dp.sum == MInt9(0)) break;
      // dp.deb;
    }
