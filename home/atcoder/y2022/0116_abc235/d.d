...
void problem() {
  ...
  auto solve() {
    long count;
    bool[long] used;
    long ketaN = N.to!string.length;
    bool smaller(long x) {
      return x.to!string.length <= ketaN;
    }

    M: for(auto q = new DList!long(1); !q.empty;) {
      bool[long] nex;
      while(!q.empty) {
        auto n = q.front; q.removeFront;
        ...
        const mul = n * A;
        if (smaller(mul) && !(mul in used)) nex[mul] = true;
        if (n >= 10 && (n % 10 != 0)) {
          auto t = n.to!string;
          t = t[$ - 1] ~ t[0..$ - 1];
          auto tn = t.to!long;
          if (smaller(tn) && !(tn in used)) nex[tn] = true;
        }
      }
