...
void problem() {
  ...
  auto M = scan!long;
  auto QN = scan!long;
  auto E = scan!long(3 * M).chunks(3).array.sort!"a[2] < b[2]";
  auto Q = scan!long(3 * QN).chunks(3).array;

  auto solve() {
    alias Query = Tuple!(long, "id", long[], "q");
    auto QQ = QN.iota.map!(i => Query(i, Q[i])).array.sort!"a.q[2] < b.q[2]";

    long qi;
    auto ans = new bool[](QN);
    auto uf = UnionFind(N + 1);
    foreach(e; E) {
      while (qi < QN && QQ[qi].q[2] < e[2]) {
        const q = QQ[qi].q;
        ans[QQ[qi].id] = !uf.same(q[0], q[1]);
        qi++;
      }
      if (!uf.same(e[0], e[1])) {
        uf.unite(e[0], e[1]);
