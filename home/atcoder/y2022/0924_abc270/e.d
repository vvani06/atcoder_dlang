...
void problem() {
  ...
  auto solve() {
    alias Work = Tuple!(long, "times", long, "amount");
    auto works = new Work[](0);
    ...
    long total = N - decr[0];
    long pre;
    foreach(t; decr.keys.sort[1..$]) {
      works ~= Work(t - pre, total);
      total -= decr[t];
      pre = t;
    }
    ...
    foreach(work; works) {
      work.deb;
      if (K >= work.amount) {
        auto times = min(work.times, K / work.amount);
        K -= work.amount * times;
        work.times -= times;
        looped += times;
      }
      ...
      if (K == 0 || work.times > 0) {
        auto ans = A.map!(a => max(0, a - looped)).array;
        foreach(i; 0..N) {
          if (K == 0) break;
        ...
        N.iota.map!(i => A[i] - ans[i]).sum.deb;
