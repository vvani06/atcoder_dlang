...
void problem() {
  auto N = scan!long;
  auto K = scan!long;
  auto S = scan!string(N).map!(s => s.map!(c => c - 'a').array).array;

  auto solve() {
    auto bits = (N + 1).iota.map!(i => 2^^i).array;

    long ans;
    foreach(i; 0..2^^N) {
      auto counts = new int[](26);
      foreach(b; 0..N) {
        if ((i & bits[b]) != bits[b]) continue;

        foreach(c; S[b]) counts[c]++;
      }
      
      ans = max(ans, counts.count!(c => c == K));
    }
