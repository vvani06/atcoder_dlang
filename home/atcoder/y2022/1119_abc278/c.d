...
void problem() {
  auto N = scan!int;
  auto QN = scan!int;
  auto Q = scan!int(QN * 3).chunks(3);

  auto solve() {
    RedBlackTree!int[int] trees;
    ...
    foreach(q; Q) {
      ...
      if (q[0] == 1) {
        trees.require(from, (new int[](0)).redBlackTree);
        trees[from].insert(to);
      } else if (q[0] == 2) {
        if (from in trees) trees[from].removeKey(to);
      } else {
        bool ans = true;
        ans &= from in trees && to in trees[from];
        ans &= to in trees && from in trees[to];
        writeln(YESNO[ans]);
      }
    }
  }
