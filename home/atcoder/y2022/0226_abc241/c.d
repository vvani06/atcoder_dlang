...
void problem() {
  ...
  auto solve() {
    foreach(x; 0..N - 5) foreach(y; 0..N) {
      if (G[y][x..x+6].count('#') >= 4) return YESNO[true];
    }
    foreach(x; 0..N) foreach(y; 0..N - 5) {
      if (6.iota.count!(d => G[y + d][x] == '#') >= 4) return YESNO[true];
    }
    foreach(x; 0..N - 5) foreach(y; 0..N - 5) {
      if (6.iota.count!(d => G[y + d][x + d] == '#') >= 4) return YESNO[true];
      if (6.iota.count!(d => G[y + d][x + 5 - d] == '#') >= 4) return YESNO[true];
    }
