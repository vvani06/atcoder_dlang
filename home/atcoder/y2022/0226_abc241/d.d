...
void problem() {
  ...
  auto solve() {
    auto highRbt = (new long[](0)).heapify!"a > b";
    auto lowRbt = (new long[](0)).heapify!"a < b";
    foreach(_; 0..QN) {
      auto qt = scan!int;
      if (qt == 1) {
        auto x = scan!long;
        ...
        auto x = scan!long;
        auto k = scan!int;
        auto r = highRbt.lowerBound(x + 1).take(k).array;
        // r.deb;
        if (r.length >= k) {
          r.back.writeln;
        } else (-1).writeln;
        ...
        auto x = scan!long;
        auto k = scan!int;
        auto r = lowRbt.lowerBound(x - 1).take(k).array;
        // r.deb;
        if (r.length >= k) {
          r.back.writeln;
        } else (-1).writeln;
