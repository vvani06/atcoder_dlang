...
void problem() {
  auto N = scan!long;
  auto A = scan!long(N * N).chunks(N).array;
  ...
  auto solve() {
    ...
    foreach(t; 1..N) {
      foreach(x; 0..t + 1) {
        auto y = t - x;
        if (x > 0) foreach(k, v; pre[y][x - 1]) {
          pre[y][x][k ^ A[y][x]] += v;
        }
        if (y > 0) foreach(k, v; pre[y - 1][x]) {
          pre[y][x][k ^ A[y][x]] += v;
        }
      }
    }
    ...
    auto suf = new V[][](N, N);
    suf[N - 1][N - 1][A[N - 1][N - 1]] = 1;
    foreach(t; 1..N) {
      foreach(x; 0..t + 1) {
        auto y = t - x;
        if (x > 0) foreach(k, v; suf[N - 1 - y][N - x]) {
          suf[N - 1 - y][N - 1 - x][k ^ A[N - 1 - y][N - 1 - x]] += v;
        }
        if (y > 0) foreach(k, v; suf[N - y][N - 1 - x]) {
          suf[N - 1 - y][N - 1 - x][k ^ A[N - 1 - y][N - 1 - x]] += v;
        }
      }
    }
    ...
    foreach(x; 0..N) {
      ...
      foreach(k, v; pre[y][x]) {
        if (x < N - 1 && k in suf[y][x + 1]) {
          ans += v * suf[y][x + 1][k];
        }
        if (y < N - 1 && k in suf[y + 1][x]) {
          ans += v * suf[y + 1][x][k];
        }
      }
    }
