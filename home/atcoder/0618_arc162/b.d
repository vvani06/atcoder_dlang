...
void problem() {
  ...
  auto solve() {
    ...
    "Yes".writeln;
    auto arr = P.dup;
    auto goal = iota(1, N + 1).array;

    int cur = 1;
    string[] ans;
    while(arr != goal) {
      auto i = arr.countUntil(cur).to!int;
      if (cur == i + 1) {
        cur++;
        continue;
      }
      ...
      auto j = cur - 1;
      ans ~= format("%s %s", i + 1, j);

      // [i, j].deb;
      auto sub = arr[0..i] ~ arr[i + 2..$];
      arr = sub[0..j] ~ arr[i .. i + 2] ~ sub[j..$];
      // arr.deb;
    }
