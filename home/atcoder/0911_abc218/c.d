...
void problem() {
  auto N = scan!long;
  auto S = cast(char[][])scan!string(N);
  auto T = cast(char[][])scan!string(N);

  auto solve() {
    auto sn = S.map!"a.count('#')".sum;
    auto tn = T.map!"a.count('#')".sum;
    if (sn != tn) return YESNO[false];

    char[][] XT;
    foreach(ref t; T) t = '.'.repeat(N).array ~ t ~ '.'.repeat(N).array;
    foreach(i; 0..N) XT ~= '.'.repeat(3 * N).array;
    XT ~= T;
    foreach(i; 0..N) XT ~= '.'.repeat(3 * N).array;
    ...
    foreach(r; 0..4) {
      foreach(y; 0..L) foreach(x; 0..L) {
        if (N.iota.all!(i => XS[i] == XT[y + i][x..x + N])) return YESNO[true];
      }
      auto ds = N.iota.map!(i => '.'.repeat(N).array).array;
