...
void problem() {
  ...
  auto solve() {
    enum char ENDC = 'z' + 1;
    int[][char] indicies;
    foreach(c; 'a'..ENDC) indicies[c] = [];
    foreach(i, c; S) indicies[c] ~= cast(int)i;

    char[] ans = S.dup;
    int last = N;
    foreach(i, c; S) {
      foreach(char rc; 'a'..c) {
        auto arr = indicies[rc].assumeSorted;
        auto candidates = arr.lowerBound(last);
        
        if (!candidates.empty) {
          const x = candidates.back;
          if (x <= i) continue;
