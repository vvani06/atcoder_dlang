...
void problem() {
  ...
  auto solve() {
    long ans = 1;
    foreach(step; 0..H+W-1) {
      const baseY = max(step - W + 1, 0);
      const baseX = min(step, W - 1);
      [baseX, baseY].deb;
      char prev = MAP[baseY][baseX];
      foreach(v; 0..step+1) {
        if (baseX - v < 0 || baseY + v >= H) break;
        
        // [baseX, baseY, v].deb;
        if (MAP[baseY + v][baseX - v] != prev && prev == '.') prev = MAP[baseY + v][baseX - v];
        if (MAP[baseY + v][baseX - v] == 'R' && prev == 'B') return 0;
        if (MAP[baseY + v][baseX - v] == 'B' && prev == 'R') return 0;
      }

      if (prev == '.') {
        // [step, baseX, baseY].deb;
        ans = (ans * 2) % MOD;
      }
    }
