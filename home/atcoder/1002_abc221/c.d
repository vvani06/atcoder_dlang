...
void problem() {
  auto N = scan;
  auto NL = N.map!(c => (c - '0').to!long).array;

  auto solve() {
    long ans;
    foreach(s; 1..N.length) {
      bool[long] isA;
      foreach(comb; N.length.iota.array.combinations(s)) {
        long[] pre, suf;
        foreach(i; 0..N.length) {
          if (comb.canFind(i)) pre ~= NL[i]; else suf ~= NL[i];
        }
      
        long a, b;
        foreach(x; pre.sort!"a > b") a = (a * 10) + x;
        foreach(x; suf.sort!"a > b") b = (b * 10) + x;
        ans = max(ans, a * b);
      }
    }
