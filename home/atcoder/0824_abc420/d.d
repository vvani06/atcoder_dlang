...
void problem() {
  ...
  auto solve() {
    ...
    long[][][] memo = new long[][][](2, H, W);
    foreach(ref mm; memo) foreach(ref m; mm) m[] = int.max;
    memo[0][sr][sc] = 0;
    alias Cursor = Tuple!(int, "inverted", int, "r", int, "c", int, "cost");
    dchar at(Cursor cur) {
      auto ret = G[cur.r][cur.c];

      if (cur.inverted == 1 && ret == 'o') return 'x'; 
      if (cur.inverted == 1 && ret == 'x') return 'o';
      return ret;
    }

    for(auto queue = [Cursor(0, sr, sc, 0)].heapify!"a.cost > b.cost"; !queue.empty;) {
      auto cur = queue.front;
      queue.removeFront();
      if (cur.cost != memo[cur.inverted][cur.r][cur.c]) continue;

      foreach(dr, dc; zip([-1, 0, 1, 0], [0, -1, 0, 1])) {
        auto nr = cur.r + dr;
        auto nc = cur.c + dc;
        if (min(nr, nc) < 0 || nr >= H || nc >= W) continue;

        auto next = Cursor(cur.inverted, nr, nc, cur.cost + 1);
        auto g = at(next);
        if (g == '#' || g == 'x') continue;

        if (g == '?') next.inverted ^= 1;
        if (memo[next.inverted][next.r][next.c].chmin(next.cost)) {
          queue.insert(next);
        }
      }
    }

    auto ans = min(memo[0][gr][gc], memo[1][gr][gc]);
