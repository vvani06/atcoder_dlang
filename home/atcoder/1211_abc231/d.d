...
void problem() {
  auto N = scan!long;
  auto M = scan!long;
  auto E = scan!long(M * 2).map!"a - 1".chunks(2);

  auto solve() {
    auto counts = new long[](N);
    auto uf = UnionFind(N);
    foreach(e; E) {
      if (++counts[e[0]] > 2) return YESNO[false];
      if (++counts[e[1]] > 2) return YESNO[false];

      if (uf.same(e[0], e[1])) return YESNO[false];
      uf.unite(e[0], e[1]);
    }
