...
void problem() {
  auto N = scan!int;
  auto P = scan!int(N - 1).map!"a - 1";
  auto C = scan!long(N);
  auto D = scan!long(N);

  auto solve() {
    auto graph = new int[][](N, 0);
    foreach(i, p; P.enumerate(1)) graph[p] ~= i;

    MInt9 dfs(int node, MInt9 acc) {
      foreach(child; graph[node]) {
        acc *= dfs(child, MInt9(1));
      }

      if (C[node] < D[node]) return MInt9(0);

      acc *= MInt9.combineNaive(C[node], D[node]);
      if (node > 0) C[P[node - 1]] += C[node] - D[node];

      // [node, acc, C[node]].deb;
      return acc;
    }

    return dfs(0, MInt9(1));
  }
