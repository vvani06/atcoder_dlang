...
void problem() {
  ...
  auto solve() {
    auto counts = S.array.sort.group.assocArray;    
    auto keys = counts.keys.sort!((a, b) => counts[a] > counts[b]);
    
    auto width = counts[keys[0]];
    auto ansStack = new string[](width);
    int index;
    foreach(k; keys) {
      foreach(_; 0..counts[k]) {
        ansStack[index] ~= k;
        index++;
        index %= width;
      }
    ...
    string ans = ansStack.joiner.to!string;
    if (ans.group.any!(g => g[1] != 1)) {
      writeln(YESNO[false]);
      return;
    }

    writeln(YESNO[true]);
    if (S.length == ans.length) writeln(ans);
  }
