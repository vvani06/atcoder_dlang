...
void problem() {
  ...
  auto solve() {
    ...
    foreach(s; S) {
      ...
      auto availableTime = s[0] - preTime;
      preTime = s[0];
      auto x = s[1];
      auto size = s[2];
      foreach(from; 0..5) {
        if (pre[from] == -1) continue;

        auto moveTime = (x - from).abs;
        if (moveTime <= availableTime) dp[x] = max(dp[x], pre[from] + size);
        foreach(to; 0..5) {
          auto stTime = (to - from).abs;
          if (stTime <= availableTime) dp[to] = max(dp[to], pre[from]);
        }
      }
      dp.deb;
    }
