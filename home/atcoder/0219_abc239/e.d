...
void problem() {
  ...
  auto QN = scan!int;
  auto X = scan!long(N);
  auto E = scan!int(2 * N - 2).chunks(2).array;
  auto Q = scan!int(2 * QN).chunks(2).array;
  ...
  auto solve() {
    ...
    auto ans = new long[][](N, 0);
    auto ranks = N.iota.map!(_ => (new long[](20)).redBlackTree!("a > b", true)).array;
    auto visited = new bool[](N);

    void dfs(int cur, int from) {
      if (visited[cur]) return;

      auto rankTree = ranks[cur];
      visited[cur] = true;
      rankTree.insert(X[cur]);
      rankTree.removeBack;
      ...
      foreach(next; graph[cur]) {
        ...
        dfs(next, cur);
        foreach(v; ranks[next]) {
          if (rankTree.back > v) break;
          
          rankTree.insert(v);
          rankTree.removeBack;
        }
      }
      ans[cur] = ranks[cur].array;
    }

    dfs(0, 0);
    return Q.map!(q => ans[q[0] - 1][q[1] - 1]).array.toAnswerString;
  }
