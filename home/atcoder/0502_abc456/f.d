...
void problem() {
  ...
  auto subSolve(int N, int K, long[] A) {
    auto segTreeOdd = SegTree!("a + b", long)(new long[](N));
    auto segTreeEven = SegTree!("a + b", long)(new long[](N));
    foreach(i; 0..N) {
      if (i % 2 == 0) segTreeOdd.add(i, A[i]);
      if (i % 2 == 1) segTreeEven.add(i, A[i]);
    }
    ...
    foreach(_; 0..3) {
      foreach(i; 0..N - K) {
        auto r = i + K - 1;
        long oddLeft = segTreeOdd.get(i);
        long oddRight = segTreeOdd.get(r);
        long evenLeft = segTreeEven.get(i);
        long evenRight = segTreeEven.get(r);

        segTreeOdd.update(i, A[i]);
        segTreeOdd.update(r, A[r]);
        segTreeEven.update(i, A[i]);
        segTreeEven.update(r, A[r]);

        ans = min(ans,
          segTreeEven.sum(i, i + K),
          segTreeOdd.sum(i, i + K),
        );

        // [i, segTreeEven.sum(i, i + K), segTreeOdd.sum(i, i + K)].deb;

        segTreeOdd.update(i, oddLeft);
        segTreeOdd.update(r, oddRight);
        segTreeEven.update(i, evenLeft);
        segTreeEven.update(r, evenRight);
      }
      K++;
    }
  ...
  auto solve() {
    foreach(_; 0..T) {
      ...
      writeln(asAnswer(subSolve(N, K, A)));
    }
  }
