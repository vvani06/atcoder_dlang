...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto LRA = iota(Q).map!(_ => tuple(scan!int, scan!int, scan!long)).array;

  auto solve() {
    auto segTree = TypicalLazySegTree!MInt9.MulAdd(N);

    foreach(l, r, a; LRA.asTuples!3) {
      segTree.apply(l - 1, r, MInt9(a));

      writeln(segTree.prod(l - 1, r));
    }
  }
...
static struct TypicalLazySegTree(S) {
  ...
  static VS withSize(S d) { return VS(d, MInt9(1)); }
  static VS[] withSize(S[] data) { return data.map!withSize.array; }

  alias MulAdd = LazySegTree!(VS, mulVSFunc, vsOne, S, addVSMapping, addFunc, sOne);
  ...
  private:
    ...
    enum mulVSFunc = (VS a, VS b) => VS(a.value * b.value, a.size + b.size);
    enum mulVSMapping = (S f, VS x) => VS(x.value * f*x.size, x.size);
    enum updateVSMapping(alias id) = (S f, VS x) => VS(f == unaryFun!id() ? x.value : f*x.size, x.size);
    enum vsZero = () => VS(MInt9(0), MInt9(0));
    enum vsOne = () => VS(MInt9(1), MInt9(1));
}
