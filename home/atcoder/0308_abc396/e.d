...
void problem() {
  auto N = scan!int;
  auto M = scan!int;
  auto XYZ = scan!int(3 * M).chunks(3).array;
  ...
  auto solve() {
    ...
    int[][int] groups;
    foreach(i; 0..N) groups[uf.root(i)] ~= i;
    bool[] defined = new bool[](N);
    auto ans = new int[](N);

    bool dfs(int cur) {
      foreach(next; graph[cur]) {
        if (defined[next.to]) {
          if ((ans[next.to] ^ ans[cur]) != next.w) return false;
        } else {
          ans[next.to] = next.w ^ ans[cur];
          defined[next.to] = true;
          if (!dfs(next.to)) return false;
        }
      }
    ...
    foreach(root, indicies; groups) {
      if (!dfs(root)) return "-1";

      foreach(bi; 0..32) {
        int bn = 2^^bi;

        auto c = indicies.count!(i => (ans[i] & bn) != 0);
        if (c > indicies.length / 2) {
          foreach(i; indicies) ans[i] ^= bn;
        }
      }
    }
