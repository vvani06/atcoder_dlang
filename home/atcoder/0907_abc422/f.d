...
void problem() {
  ...
  auto solve() {
    ...
    enum INF = long.max / 3;
    alias Cursor = Tuple!(int, "node", long, "w");

    foreach(goal; 0..N) {

      long subSolve(long finalWeight) {
        long[] x = INF.repeat(N).array;
        long[] w = 0L.repeat(N).array;

        for(auto heap = [Cursor(goal, finalWeight)])
