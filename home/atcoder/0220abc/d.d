void main() {
  problem();
}

void problem() {
  auto X = scan.map!(c => c - '0').array.reverse.array;
  auto M = BigInt(scan.to!long);

  auto solve() {
    if (X.length == 1) {
      return X[0] <= M ? 1 : 0;
    }

    long decimal;
    foreach(c; X) decimal = decimal.max(c);
 
    BigInt based(long decimal) {
      BigInt num = 0;
      BigInt dr = BigInt(decimal);
      BigInt k = 1;
      foreach(i, x; X) {
        num += k * x;
        k *= dr;
      }
 
      return num;
    }

    const border = binarySearch(&based, (BigInt a) => a > M, 0, long.max);

    return max(0, border - decimal - 1);
  }

  static if (is(ReturnType!(solve) == void)) solve(); else solve().writeln;
}

// ----------------------------------------------

import std.bigint;
import std.stdio, std.conv, std.array, std.string, std.algorithm, std.container, std.range, core.stdc.stdlib, std.math, std.typecons, std.numeric, std.traits, std.functional;
T[][] combinations(T)(T[] s, in int m) {   if (!m) return [[]];   if (s.empty) return [];   return s[1 .. $].combinations(m - 1).map!(x => s[0] ~ x).array ~ s[1 .. $].combinations(m); }
string scan(){ static string[] ss; while(!ss.length) ss = readln.chomp.split; string res = ss[0]; ss.popFront; return res; }
T scan(T)(){ return scan.to!T; }
T[] scan(T)(long n){ return n.iota.map!(i => scan!T()).array; }
void deb(T ...)(T t){ debug writeln(t); }
alias Point = Tuple!(long, "x", long, "y");
Point invert(Point p) { return Point(p.y, p.x); }
ulong MOD = 10^^9 + 7;
long[] divisors(long n) { long[] ret; for (long i = 1; i * i <= n; i++) { if (n % i == 0) { ret ~= i; if (i * i != n) ret ~= n / i; } } return ret.sort.array; }
bool chmin(T)(ref T a, T b) { if (b < a) { a = b; return true; } else return false; }
bool chmax(T)(ref T a, T b) { if (b > a) { a = b; return true; } else return false; }

// -----------------------------------------------

T binarySearch(T, K)(K delegate(T) fn, bool delegate(K) cond, T l, T r) {
  auto lb = l;
  auto ub = r;
  const T TWO = 2;

  while(true) {
    static if (is(T == float) || is(T == double) || is(T == real)) {
      if(lb.approxEqual(ub, 1e-08, 1e-08)) break;
    } else {
      if(lb == ub || lb + 1 == ub) break;
    }
    const half = (ub + lb) / TWO;
    const halfValue = fn(half);

    if (cond(halfValue)) {
      ub = half;
    } else {
      lb = half;
    }
  }

  return ub;
}