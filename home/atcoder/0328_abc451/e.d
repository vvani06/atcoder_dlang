...
void problem() {
  auto N = scan!int;
  auto A = (N - 1).iota.map!(i => scan!int(N - 1 - i)).array;

  auto solve() {
    alias Edge = Tuple!(int, "from", int, "to", int, "cost");
    Edge[] edges;
    foreach(u; 0..N - 1) {
      foreach(v, w; A[u].enumerate(u + 1)) {
        edges ~= Edge(u, v, w);
      }
    }
    ...
    foreach(edge; edges.sort!"a[2] < b[2]") {
      if (uf.same(edge.from, edge.to)) continue;

      uf.unite(edge.from, edge.to);
      graph[edge.from] ~= edge;
      graph[edge.to] ~= Edge(edge.to, edge.from, edge.cost);
    }
    ...
    foreach(s; 0..N - 1) {
      ...
      void dfs(int cur, int pre, int cost) {
        ...
        foreach(edge; graph[cur]) {
          ...
          dfs(edge.to, cur, cost + edge.cost);
        }
      }
      dfs(s, s, 0);
      if (costs[s + 1..$] != A[s]) return false;
    }
    return true;
  }
