...
void problem() {
  auto N = scan!int;
  auto Q = scan!int;
  auto UV = scan!int(2 * Q).chunks(2).array;

  auto solve() {
    auto uf = UnionFind(N * 2);
    foreach(i; 0..N) {
      uf.setExtra(i, UnionFindExtra(0, 1));
      uf.setExtra(i + N, UnionFindExtra(1, 0));
    }

    int calcMinNodes(int n) {
      return min(uf.extra(n).minElement, uf.extra((n + N) % N).minElement);
    }

    bool broken;
    int ans;
    foreach(u, v; UV.asTuples!2) {
      u--;
      v--;

      auto froms = new int[](0);
      if (!broken) {
        froms = [u, v].map!(i => calcMinNodes(i)).array;
        if (uf.same(u, v + N) && uf.same(u + N, v)) froms.length = 1;
        
        uf.unite(u, v + N);
        uf.unite(u + N, v);
        if (uf.same(u, u + N) || uf.same(v, v + N)) broken = true;
      }

      if (broken) {
        writeln(-1);
      } else {
        auto to = calcMinNodes(u);
        ans += to - froms.sum;
        writeln(ans);
      }
    }
...
struct UnionFindExtra {
  int b, w;
  int minElement() { return min(b, w); }
  UnionFindExtra merge(UnionFindExtra other) { return UnionFindExtra(b + other.b, w + other.w); }  
}
