...
void problem() {
  auto N = scan!int;
  auto XH = scan!real(2 * N).chunks(2).array;

  auto solve() {
    bool isOk(real baseY) {
      real theta = -PI;

      foreach(xh; XH) {
        real x = xh[0];
        real h = xh[1];
        auto t = atan2(h - baseY, x);

        if (theta > t || !t.isClose(t, 1e-12, 1e-12)) return false;
        theta = t;
      }
      return true;
    }
    ...
    if (isOk(0)) {
      ...
      auto ans = binarySearch(&isOk, 1e40, 0.0L);
      writefln("%.16f", ans);
    }
  }
...
T binarySearch(T, K)(K delegate(T) fn, bool delegate(K) cond, T l, T r) {
  ...
  bool again() {
    static if (is(T == float) || is(T == double) || is(T == real)) {
      return !ng.isClose(ok, 1e-12, 1e-12);
    } else {
      return abs(ng - ok) > 1;
    }
  }
