...
void problem() {
  ...
  auto solve() {
    ...
    auto counts = new int[long][int];
    foreach(tl; T.map!"a.length.to!int".array.sort.uniq) {
      counts.require(tl, new int[long]);
      foreach(i; 0..l - tl + 1) {
        auto h = rh.get(i, i + tl);
        counts[tl][h]++;
      }
    }

    foreach(t; T) {
      auto trh = RollingHash(t);
      auto tl = t.length.to!int;
      auto th = trh.get(0, tl);

      int ans;
      ans = counts[tl].get(th, 0);
      ans.writeln;
    }
  }
...
struct RollingHash {
  ...
  this(T)(T[] arr) {
    foreach(i, a; arr) {
      hashes ~= (mul(hashes[i], base) + a.hashOf) % MOD;
