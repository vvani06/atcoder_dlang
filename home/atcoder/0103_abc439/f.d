...
void problem() {
  ...
  auto solve() {
    auto limit = N + 1;
    auto spawns = SegTree!("a + b", MInt9)(new MInt9[](limit));
    auto decs = SegTree!("a + b", MInt9)(new MInt9[](limit));
    auto incs = SegTree!("a + b", MInt9)(new MInt9[](limit));

    MInt9 ans;
    foreach(a; P) {
      ans += decs.sum(1, a) + incs.sum(1, a);

      spawns.update(a, MInt9(1));
      decs.update(a, incs.sum(a, limit) + decs.sum(a, limit));
      incs.update(a, spawns.sum(1, a) + decs.sum(1, a) + incs.sum(1, a));
    }

    // P.map!(i => spawns.get(i)).deb;
    // P.map!(i => decs.get(i)).deb;
    // P.map!(i => incs.get(i)).deb;

    return decs.sum(1, limit);
  }
