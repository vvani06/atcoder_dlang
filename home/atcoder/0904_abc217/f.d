...
void problem() {
  ...
  auto solve() {
    const N2 = N * 2;
    bool[Tuple!(long, long)] friend;
    foreach(p; P) {
      p[0]--; p[1]--;
      friend[tuple(p[0], p[1])] = true;
    }

    enum long MOD = 998_244_353;
    long[bool[]] dp;
    long[bool[]] preDp;
    dp[false.repeat(N2).array.idup] = 1;
    foreach(_; 0..N) {
      preDp.clear;
      swap(dp, preDp);
      foreach(used, pre; preDp) {
        auto left = used.countUntil(false);
        foreach(right; left + 1..N2) {
          if (used[right]) continue;

          if (tuple(left, right) in friend) {
            auto to = used.dup;
            to[left] = to[right] = true;
            const ito = to.idup;
            if (ito in dp) dp[ito] = (dp[ito] + pre) % MOD; else dp[ito] = pre;
          }
          left = right;
        }
      }
    ...
    auto goal = true.repeat(N2).array.idup;
    return goal in dp ? dp[goal] : 0;
  }
