...
void problem() {
  auto N = scan!int;
  auto Q = scan!int(3 * scan!int).chunks(3).array;

  auto solve() {
    auto uf = UnionFind(N);

    int rbtTake(T)(T rbt, int n) {
      int[] taken;
    ...
    foreach(q; Q) {
      if (q[0] == 1) {
        ...
        if (size < k) {
          ...
          auto nodes = uf.nodes[uf.root(v)];
          nodes.take(1).deb;
          auto ans = rbtTake(nodes, k);
          writeln(ans + 1);
        }
      }
    }
...
struct UnionFind {
  ...
  bool unite(int x, int y, long w = 0) {
    ...
    nodes[rootX].insert(nodes[rootY].array);
    nodes[rootY].clear;
    roots[rootY] = rootX;
    return true;
  }
