...
void problem() {
  ...
  auto solve() {
    auto fermet = FermetCalculator!998_244_353(NN);
    auto factrial = fermet.factrial.map!(f => MInt9(f)).array;
    MInt9 ans = factrial[NN];
    ...
    foreach(x; N..NN - N + 2) {
      ...
      MInt9 dec = 1;
      dec *= MInt9(fermet.combine(h, N - 1)) * factrial[N - 1];
      dec *= MInt9(fermet.combine(w, N - 1)) * factrial[N - 1];
      dec *= factrial[(N - 1) * (N - 1)];
      ans -= dec * mNN;
    }
