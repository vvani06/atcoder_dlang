...
void problem() {
  ...
  long solve(bool startsWithPositive) {
    long ans;
    long[] acc; acc.length = N + 1;
    foreach(i; 1..N+1) {
      auto value = acc[i - 1] + A[i-1];
      if (i == 1) {
        if (startsWithPositive && value <= 0) {
          ans += value.abs + 1;
          value = 1;
        }
        if (!startsWithPositive && value >= 0) {
  ...
  min(solve(true), solve(false)).writeln;
}
