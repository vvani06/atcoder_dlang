...
void problem() {
  ...
  auto solve() {
    ...
    auto magmas = DList!GridPoint();
    foreach(y; 0..H) foreach(x; 0..W) if (G[y][x] == '@') magmas ~= GridPoint(x, y);

    long step;
    for(auto q = new DList!GridPoint(START); !q.empty;) {
      step++;
      if (step % K == 0) {
        foreach(m; magmas.array) {
          magmas.removeFront;
          foreach(a; m.around(rect)) {
            if (a.of(G) == '.' && !a.of(visited)) {
              G[a.y][a.x] = '@';
              magmas.insertBack(a);
            }
          }
      ...
      while(!q.empty) {
        ...
        if (p == GOAL) return YESNO[true];

        foreach(a; p.around(rect)) {
          if (a.of(G) == '.' && !a.of(visited)) next ~= a;
        }
      }
