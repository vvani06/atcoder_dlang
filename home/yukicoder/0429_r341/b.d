...
void problem() {
  ...
  auto solve() {
    ...
    dp[0][0][G[0][0..1]] = 1;
    const stepMax = W+H-1;
    const stepHalf = stepMax / 2;
    foreach(step; 1..W+H-1) {
      foreach(x; 0..W) {
        const y = step - x;
        if (y >= H || y < 0) continue;

        // [x, y].deb;
        foreach(pre; [[x - 1, y], [x, y - 1]]) {
          if (pre.minElement < 0) continue;

          if (step < stepHalf) {
            foreach(k, v; dp[pre[1]][pre[0]]) {
              dp[y][x][k ~ G[y][x]] += v;
              dp[y][x][k ~ G[y][x]] %= MOD;
            }
          } else if(stepMax % 2 == 1 && step == stepHalf) {
            foreach(k, v; dp[pre[1]][pre[0]]) {
              dp[y][x][k] += v;
              dp[y][x][k] %= MOD;
            }
