...
void problem() {
  ...
  auto solve() {
    auto expected = new bool[](N + 1);
    foreach(i; 1..N + 1) expected[i] = (N / i) % 2 == 1 ? true : false;

    auto actual = new bool[](N + 1);
    foreach(a; A) actual[a] = true;

    int ans;
    foreach_reverse(i; 1..N + 1) {
      if (actual[i] != expected[i]) {
        ans++;
        foreach(d; i.divisors) expected[d] ^= true;
      }
    }
