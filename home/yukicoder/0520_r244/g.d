...
void problem() {
  ...
  auto V = scan!int;
  auto C = scan!int;
  auto VW = scan!int(2 * N).chunks(2).array;

  auto solve() {
    int[][int] items;
    foreach(vw; VW.multiSort!("a[0] < b[0]", "a[1] < b[1]")) {
      items[vw[0]] ~= vw[1];
    }

    long[5001] dp;
    long ans;
    foreach(v, warr; items) {
      foreach(w; warr[0..$ - 1]) {
        foreach_reverse(fv; 0..V + 1 - v) {
          long nv = dp[fv] + w + C;
          if (dp[fv + v].chmax(nv)) ans = max(ans, nv);
        }
      }
      auto w = warr[$ - 1];
      foreach_reverse(fv; 0..V + 1 - v) {
        for(int i = 1; fv + v*i <= V; i++) {
          long nv = dp[fv] + w*i + C;
          if (dp[fv + v*i].chmax(nv)) ans = max(ans, nv);
        }
      }
    }
