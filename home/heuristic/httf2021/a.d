...
void problem() {
  ...
  auto solve() {
    ...
    while(rest > 0) {
      long perf = long.max;
      Polyomino bestPolyomino;
      int bestX, bestY, bestFills;
      
      SEL:foreach(p; POLYOMINOS[1..$]) {
        foreach(int ox; 0..N - p.w + 1) SLIDEX:foreach(int oy; 0..N - p.h + 1) {
          int fills;
          long score = 2L^^20 * p.cost;
          foreach(ly, xs; p.shape) {
            int y = oy + cast(int)ly;
            foreach(x; xs) {
              if (grid[y][ox + x]) continue SLIDEX;
              if (GridPoint(ox + x, y) in goals) fills++;
            }
          }
      ...
      // rest.deb;
      if (bestFills == 0) break;

      rest -= bestFills; 
      foreach(y, xs; bestPolyomino.shape) {
        foreach(x; xs) {
          grid[bestY + cast(int)y][bestX + x] = true;
          // if (GridPoint(bestX + x, bestY + cast(int)y) in goals) GridPoint(bestX + x, bestY + cast(int)y).deb;
          goals.remove(GridPoint(bestX + x, bestY + cast(int)y));
        }
      }
      // bestPolyomino.deb;
      ans ~= Action(bestPolyomino, bestX, bestY);
    }
